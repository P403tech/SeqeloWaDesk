...
// ───────── Instaflow (Instagram) → WaDesk push ─────────
// The separate Instaflow deployment pushes new IG events here; the controller
// self-guards on the X-Instaflow-Secret shared secret. Serves /api/instaflow/inbound.
Route::post('/instaflow/inbound',
...
// Delivery/read receipt for NON-broadcast sends (Quick Send / /chat /
// team-inbox) — updates the messages row by its stored WhatsApp message id so
// ticks progress past "sent" (Quick Send Delivered count was stuck at 0).
Route::post('/chat/message-status',
    [\App\Http\Controllers\ChatController::class, 'nodeChatMessageStatus'])
    ->name('chat.node.message-status');
...
// ───────── Multi-service booking (Node runtime, X-Node-Token) ─────────
Route::get('/booking/types',    [\App\Http\Controllers\BookingApiController::class, 'types'])->name('api.booking.types');
Route::get('/booking/slots',    [\App\Http\Controllers\BookingApiController::class, 'slots'])->name('api.booking.slots');
Route::post('/booking/reserve', [\App\Http\Controllers\BookingApiController::class, 'reserve'])->name('api.booking.reserve');
Route::post('/booking/book',    [\App\Http\Controllers\BookingApiController::class, 'book'])->name('api.booking.book');
...
// Outbound AI call: the Node bridge minted an SDP offer and hands it here to
// actually dial the customer via Meta's calls API.
Route::post('/waba-call/bridge-offer',
    [\App\Http\Controllers\FlowNodeActionsController::class, 'wabaCallBridgeOffer'])
    ->name('api.waba-call.bridge-offer');
