...
*/
Route::middleware(['guest', \App\Http\Middleware\NoStoreResponse::class])->group(function () {
    ...
    // Registration WhatsApp OTP verify step (only reachable mid-signup; the
    // pending account lives in the session until the code is confirmed).
    Route::get('/register/verify',           [AuthPagesController::class, 'showRegisterOtp'])->name('register.verify');
    Route::post('/register/verify',          [AuthPagesController::class, 'verifyRegisterOtp'])->middleware('throttle:10,1');
    Route::post('/register/verify/resend',   [AuthPagesController::class, 'resendRegisterOtp'])
        ->middleware('throttle:5,1')->name('register.verify.resend');
...
// Live CSRF token — matches the CURRENT session cookie. resources/js/csrf-guard.js
// fetches this on page load + bfcache restore and rewrites a cached/stale form
// _token (and the csrf meta) with it, so a login page served from browser/CDN
// cache still POSTs a valid token instead of 419-ing "session expired". Outside
// the guest group so it works for both signed-out and signed-in users; no-store
// so the token itself is never cached.
...
*/
// Public marketing HTML — wrapped in NoStoreResponse so a mobile browser (or a
// CDN edge) never serves a STALE cached copy of the page. A cached page still
// references the PREVIOUS build's hashed CSS/JS filenames; after a deploy those
// files are gone, so the stylesheet 404s and the page renders as raw, unstyled
// text — intermittently, and worst on mobile where HTML is cached hardest and
// hard-refresh is rare. Same class of bug (and same fix) as the auth pages.
// sitemap.xml / robots.txt are left cacheable — they carry no hashed-asset refs.
Route::middleware(\App\Http\Middleware\NoStoreResponse::class)->group(function () {
...
// SEO sitemap/robots — safe to cache (no hashed-asset references).
...
Route::prefix('legal')->name('frontend.legal.')->middleware(\App\Http\Middleware\NoStoreResponse::class)->group(function () {
...
// Team-Inbox PWA — the SEPARATE installable app scoped to /team-inbox (agents
// install it for background push). Public manifest so "Add to Home Screen"
// can fetch it. The push subscribe/key endpoints live under team-inbox/api.
Route::get('/team-inbox-manifest.json', [\App\Http\Controllers\TeamInboxPwaController::class, 'manifest'])->name('team-inbox.manifest');
...
// Public booking management — capability-token pages (no auth). GET only
// renders (link-preview safe); mutations are POST. Token is unguessable.
Route::middleware('throttle:60,1')->group(function () {
    $bm = \App\Http\Controllers\BookingManageController::class;
    Route::get('/b/manage/{token}',            [$bm, 'show'])->name('booking.manage.show');
    Route::get('/b/manage/{token}/slots',      [$bm, 'slots'])->name('booking.manage.slots');
    Route::get('/b/manage/{token}/reschedule', [$bm, 'rescheduleForm'])->name('booking.manage.reschedule.form');
    Route::post('/b/manage/{token}/reschedule', [$bm, 'reschedule'])->name('booking.manage.reschedule');
    Route::post('/b/manage/{token}/cancel',    [$bm, 'cancel'])->name('booking.manage.cancel');
});

// Booking deposit payment callback — gateway redirects the customer here after
// paying; verifies + commits the booking (self-contained, not package checkout).
Route::match(['get', 'post'], '/b/pay/callback/{gateway}', [\App\Http\Controllers\BookingPaymentController::class, 'callback'])
    ->name('booking.pay.callback');

// Public invoice view + PDF — token is the only capability (unguessable). This
...
Route::middleware('throttle:120,1')->group(function () {
    ...
    Route::get('/b/{token}/pdf', [$br, 'publicPdf'])->name('brief.public.pdf');
    // AI-CRM Phase 7 — public Proposal/Estimate share page (token-only).
    $sd = \App\Http\Controllers\SalesDocsController::class;
    Route::get('/q/{token}',          [$sd, 'publicShow'])->name('salesdoc.public.show');
...
// Native-checkout CUSTOMER payment webhook — one per gateway slug. Machine-to-
// machine from the merchant's gateway, authed by the driver's HMAC signature
// (CSRF-exempt in bootstrap/app.php). Closes the money loop → WaOrder paid.
Route::post('/wa/checkout/webhook/{slug}', [\App\Http\Controllers\CustomerPaymentWebhookController::class, 'handle'])
    ->name('wa.checkout.webhook');
// Public customer return/thank-you page after paying — confirms server-side
// (webhook fallback that also works on hosts the gateway can't reach) + shows a
// proper result page. Resolved by the order's unguessable recovery_token.
Route::get('/wa/checkout/return/{slug}/{token}', [\App\Http\Controllers\CustomerCheckoutReturnController::class, 'handle'])
    ->name('wa.checkout.return');
...
Route::prefix('widget/{token}')->where(['token' => '[A-Za-z0-9]{16,80}'])->group(function () {
    ...
    // NOTE: many nginx deployments serve any request ending in `.js`
    // straight off disk (`location ~* \.js$ { try_files $uri =404; }`)
    // BEFORE it ever reaches PHP — so `/widget/{token}/embed.js` returned
    // nginx's own "404 Not Found" and the embed silently failed on the
    // customer's site (while /chat, which has no extension, worked). The
    // extension-less `/embed` alias below is the one the snippet points at
    // now: it falls through to index.php like every other Laravel route,
    // so embedding works regardless of the host's static-file rules. The
    // `.js` route stays for backward-compat with already-pasted snippets
    // on correctly-configured servers.
    Route::get('/embed',        [$w, 'embedJs'])->name('widget.public.embed');
    Route::get('/embed.js',     [$w, 'embedJs'])->name('widget.public.embed_js');
...
// Shopify MANDATORY GDPR compliance webhooks (customers/data_request,
// customers/redact, shop/redact). One fixed URL for all three — dispatched by
// the X-Shopify-Topic header. Unauthenticated + no {secret}: Shopify's app
// checker probes it with no store installed, so it must verify HMAC (against the
// app client secret) and return 401/200 without any prior row lookup.
Route::post('/shopify/compliance',
    [\App\Http\Controllers\ShopifyController::class, 'compliance'])
...
Route::post('/payment/callback/{gateway}',
    [\App\Http\Controllers\CheckoutController::class, 'callback']);
// Paddle built-in checkout — renders our own Paddle.js overlay for a created
// transaction so payment works without depending on the Paddle dashboard
// "default payment link". Public (the buyer is redirected here mid-checkout).
Route::get('/payment/paddle/checkout/{order}',
    [\App\Http\Controllers\CheckoutController::class, 'paddleCheckout'])
    ->whereNumber('order')->name('payment.paddle.checkout');
...
Route::middleware('auth')->group(function () {
    ...
    // unlike /register/workspace which is part of onboarding). Gated to
    // admin/owner — an invited AGENT/VIEWER (or manager) must NOT be able to
    // spin up new workspaces from inside a workspace they were added to (client
    // report: "an agent I invited could create workspaces"). The gate is
    // hierarchy-based, so owners still pass; agents/viewers are redirected back
    // to /team-inbox. A brand-new user's FIRST workspace still comes through the
    // onboarding /register/workspace route above, which stays open.
    Route::get('/workspaces/create',  [\App\Http\Controllers\WorkspacesController::class, 'create'])->middleware('workspace.role:admin')->name('workspaces.create');
    Route::post('/workspaces',        [\App\Http\Controllers\WorkspacesController::class, 'store'])->middleware('workspace.role:admin')->name('workspaces.store');
    ...
    Route::name('user.')->group(function () {
        /*
         * Shared media upload for the template send-override panel
         * (resources/js/charts/template-live-mapping.js). Campaigns, broadcasts,
         * scheduled, team-inbox and chat all reuse that panel and POST the
         * picked/dropped file here; the returned public URL is dropped into the
         * send so WhatsApp can fetch the header media. Previously the panel
         * pointed at the flow-builder's /flows/api/upload-media (admin-only,
         * prefixed) which 404'd from every other surface. This top-level route
         * is workspace-scoped at manager+, the lowest role that reaches the panel.
         */
        Route::post('/api/upload-media', [\App\Http\Controllers\FlowsController::class, 'apiUploadMedia'])
            ->middleware('workspace.role:manager')->name('api.upload-media');
        ...
        Route::middleware('workspace.role:manager')->group(function () {
            ...
            // Contact 360 profile + activity timeline (Phase 1). Numeric-constrained
            // so it never shadows /contacts/sample-csv, /contacts/search, etc.
            Route::get('/contacts/{id}',                  [ContactsController::class, 'show'])->whereNumber('id')->name('contacts.show');
            ...
            Route::post('/contacts/bulk-tag',             [ContactsController::class, 'bulkTag'])->name('contacts.bulk-tag');
        ...
        Route::prefix('broadcasts')->middleware('workspace.role:admin')->name('broadcasts.')->group(function () {
            ...
            Route::get('/{id}/export',       [BroadcastsController::class, 'export'])->whereNumber('id')->name('export');
        ...
        Route::prefix('meta-ads')->middleware('workspace.role:admin')->name('meta-ads.')->group(function () {
            ...
            // Advanced targeting — live autocomplete + saved-audience list.
            // Fixed paths, declared BEFORE the /{id} catch-all below.
            Route::get('/targeting-search', [MetaAdsController::class, 'targetingSearch'])->name('targeting-search');
            Route::get('/audiences',        [MetaAdsController::class, 'audiences'])->name('audiences');
        ...
        Route::prefix('flows')->middleware('workspace.role:admin')->name('flows.')->group(function () {
            ...
            // Import an existing Instaflow (Instagram) flow into WaDesk + open it
            // in the builder for editing. Round-trips back on save.
            Route::get('/import-ig/{igId}',  [\App\Http\Controllers\FlowsController::class, 'importIgFlow'])->whereNumber('igId')->name('import-ig');
            ...
            Route::post('/{id}/rename',      [\App\Http\Controllers\FlowsController::class, 'rename'])->whereNumber('id')->name('rename');
            ...
            // Test Runner — fire a Webhook node's real request from "Simulate a
            // chat" (SSRF-guarded), so operators can confirm their endpoint is hit.
            Route::post('/api/test-webhook', [\App\Http\Controllers\FlowsController::class, 'apiTestWebhook'])->name('api.test-webhook');
            ...
            Route::get('/api/call-assistants', [\App\Http\Controllers\FlowsController::class, 'apiCallAssistants'])->name('api.call-assistants');
        ...
        Route::prefix('templates')->middleware('workspace.role:manager')->name('templates.')->group(function () {
            ...
            // (button shown only in remote-Instaflow mode — hidden for the native addon).
            Route::post('/sync-instaflow',     [TemplatesController::class, 'syncFromInstaflow'])->name('sync-instaflow');
        ...
        Route::prefix('devices')->middleware('workspace.role:admin')->name('devices.')->group(function () {
            ...
            // BSP per-client billing choice — "Meta bills me" vs "bill me through us".
            Route::post  ('/billing-mode',          [DevicesController::class, 'setBillingMode'])->name('billing-mode');
            ...
            // stays on Instaflow; these manage this workspace's mirror rows:
            //   available     — IG accounts on Instaflow not yet linked (picker)
            //   link          — upsert a mirror row (also the per-row Refresh)
            //   connect-start — mint a popup ticket to connect a NEW account
            //   return        — no-opener fallback for the connect popup
            //   {id}/unlink   — drop this workspace's mirror row
            Route::get   ('/instagram/available',     [DevicesController::class, 'instagramAvailable'])->name('instagram.available');
            Route::post  ('/instagram/link',          [DevicesController::class, 'instagramLink'])->name('instagram.link');
            ...
            // Register THIS number on the Cloud API — fixes sends failing with
            // (#133010) Account not registered on a manually/popup-connected number.
            Route::post  ('/waba/{id}/register',    [DevicesController::class, 'wabaRegister'])->whereNumber('id')->name('waba.register');
            Route::post  ('/waba/{id}/sync-history', [DevicesController::class, 'wabaSyncHistory'])->whereNumber('id')->name('waba.sync-history');
            ...
            Route::post  ('/waba/{id}/token-refresh', [DevicesController::class, 'wabaRefreshToken'])->whereNumber('id')->name('waba.token-refresh');
        ...
        Route::post('/connect/wa-store/sms',     [\App\Http\Controllers\WaConnectController::class, 'saveSms'])->name('connect.wa-store.sms');
        ...
        Route::prefix('store')->middleware('workspace.role:admin')->name('store.')->group(function () {
            ...
            // Invoices — rendered INSIDE the store dashboard (sidebar + panel), gated by the invoice plan feature.
            Route::get('/invoices', fn () => view('user.store.invoices'))->middleware('plan:auto_invoice')->name('invoices');
            // Products
            ...
            Route::post('/products/{id}/send', [\App\Http\Controllers\WaProductController::class, 'sendToNumber'])->whereNumber('id')->name('products.send');
            ...
            // Native-checkout: send the customer a hosted pay link via a chosen
            // merchant gateway (any of the 32). Works on WABA + Baileys.
            Route::post('/orders/{id}/send-checkout', [\App\Http\Controllers\WaOrderController::class, 'sendCheckout'])->whereNumber('id')->name('orders.send-checkout');
            // Merchant CHECKOUT-GATEWAY settings (per-workspace keys for the 32
            // hosted-checkout drivers → wa_merchant_gateways). Distinct from
            // /store/payments (that page is Meta's native WhatsApp Pay).
            Route::get('/gateways',                 [\App\Http\Controllers\MerchantGatewaysController::class, 'index'])->name('gateways.index');
            Route::post('/gateways/{slug}',         [\App\Http\Controllers\MerchantGatewaysController::class, 'save'])->name('gateways.save');
            Route::post('/gateways/{slug}/toggle',  [\App\Http\Controllers\MerchantGatewaysController::class, 'toggle'])->name('gateways.toggle');
            Route::delete('/gateways/{slug}',       [\App\Http\Controllers\MerchantGatewaysController::class, 'destroy'])->name('gateways.destroy');
            ...
            // Plan-gated: access_whatsapp_pay (was reachable with only the admin role check).
            Route::middleware('plan:access_whatsapp_pay')->group(function () {
        ...
        Route::prefix('auto-reply')->middleware('workspace.role:admin')->name('auto-reply.')->group(function () {
            ...
            // Manual "Away mode" switch — flips workspaces.inbox_away so `away`
            // trigger-type auto-replies fire until it is switched off.
            Route::post('/away-mode',      [$ar, 'toggleAway'])->name('away-mode');
        });
        ...
        Route::prefix('wa-calling')->middleware(['workspace.role:manager', 'plan:access_waba_calling'])->name('wa-calling.')->group(function () {
            ...
            // Operator-initiated recording. Calls are NOT recorded automatically;
            // this starts capture mid-call from the moment it is pressed.
            Route::post('/calls/{id}/record',      [$wc, 'startRecording'])->whereNumber('id')->name('record');
        ...
        Route::middleware('workspace.role:admin')->group(function () {
            ...
            Route::middleware('plan:integration_slack')->group(function () {
                ...
                Route::post('/slack/{id}/settings',   [$sl, 'settings'])->whereNumber('id')->name('slack.settings');
            ...
            // Booking Types (multi-service) — the 3-step wizard + list.
            $btc = \App\Http\Controllers\BookingTypeController::class;
            Route::middleware('plan:access_appointment_booking')->group(function () use ($ap, $btc) {
                Route::post('/appointments/booking-types/preview-slots', [$ap, 'previewSlots'])->name('appointments.booking-types.preview-slots');
                Route::get('/appointments/booking-types/list',            [$btc, 'apiList'])->name('appointments.booking-types.list');
                Route::get('/appointments/booking-types',                 [$btc, 'index'])->name('appointments.booking-types.index');
                Route::get('/appointments/booking-types/create',          [$btc, 'create'])->name('appointments.booking-types.create');
                Route::post('/appointments/booking-types',                [$btc, 'store'])->name('appointments.booking-types.store');
                Route::get('/appointments/booking-types/{bookingType}/edit', [$btc, 'edit'])->whereNumber('bookingType')->name('appointments.booking-types.edit');
                Route::put('/appointments/booking-types/{bookingType}',    [$btc, 'update'])->whereNumber('bookingType')->name('appointments.booking-types.update');
                Route::delete('/appointments/booking-types/{bookingType}', [$btc, 'destroy'])->whereNumber('bookingType')->name('appointments.booking-types.destroy');
                Route::post('/appointments/booking-types/{bookingType}/toggle', [$btc, 'toggle'])->whereNumber('bookingType')->name('appointments.booking-types.toggle');
            });
            ...
            // Unified social publishing — All Posts + Calendar across IG/FB/TikTok.
            $sp = \App\Http\Controllers\SocialPostsController::class;
            Route::get('/social/posts',      [$sp, 'posts'])->name('social.posts');
            Route::get('/social/posts/data', [$sp, 'postsData'])->name('social.posts.data'); // live poller JSON
            Route::get('/social/calendar',   [$sp, 'calendar'])->name('social.calendar');
            Route::post('/social/schedule',  [$sp, 'schedule'])->name('social.schedule'); // drawer compose+schedule

            // Auto-Invoice — merchant list / detail / generate / download / settings.
            $inv = \App\Http\Controllers\InvoicesController::class;
            $ivs = \App\Http\Controllers\InvoiceSettingsController::class;
            Route::middleware('plan:auto_invoice')->group(function () use ($inv, $ivs) {
                Route::get('/invoices',                 [$inv, 'index'])->name('invoices.index');
                Route::post('/invoices',                [$inv, 'store'])->name('invoices.store');
                // AI-CRM Phase 2.1 — free-form (order-less) invoice builder. `create`
                // is registered before the {id} route (which is whereNumber anyway).
                Route::get('/invoices/create',          [$inv, 'createManual'])->name('invoices.create');
                Route::post('/invoices/manual',         [$inv, 'storeManual'])->name('invoices.manual');
                Route::get('/invoices/settings',        [$ivs, 'edit'])->name('invoices.settings');
                Route::post('/invoices/settings',       [$ivs, 'save'])->name('invoices.settings.save');
                Route::post('/invoices/template',       [$ivs, 'createTemplate'])->name('invoices.template.create');
                Route::get('/invoices/{id}',            [$inv, 'show'])->whereNumber('id')->name('invoices.show');
                Route::get('/invoices/{id}/pdf',        [$inv, 'pdf'])->whereNumber('id')->name('invoices.pdf');
                Route::post('/invoices/{id}/resend',    [$inv, 'resend'])->whereNumber('id')->name('invoices.resend');

                // AI-CRM Phase 2.2 — payment ledger (record money-in, outstanding, aging).
                $pay = \App\Http\Controllers\PaymentsController::class;
                Route::get('/payments',                 [$pay, 'index'])->name('payments.index');
                Route::post('/payments',                [$pay, 'store'])->name('payments.store');
            });
            ...
            Route::middleware('plan:access_wa_forms')->group(function () use ($waf) {
            ...
            Route::middleware('plan:access_wa_links')->group(function () use ($wcl) {
            ...
            // Plan-gated: the website chatbot-widget builder requires access_chatbot_widgets
            // (was previously reachable with only the admin role check).
            Route::middleware('plan:access_chatbot_widgets')->group(function () use ($cbw) {
        ...
        Route::get('/analytics', [UserPagesController::class, 'analytics'])
            ->middleware(['workspace.role:manager', 'plan:access_analytics'])->name('analytics');
        Route::get('/analytics/export', [UserPagesController::class, 'analyticsExport'])
            ->middleware(['workspace.role:manager', 'plan:access_analytics'])->name('analytics.export');

        // AI usage / spend dashboard — token burn, provider/model breakdown,
        // plan-vs-BYOK split and monthly-cap progress for the current workspace.
        Route::get('/ai-usage', [\App\Http\Controllers\AiUsageController::class, 'index'])
            ->middleware('workspace.role:manager')->name('ai-usage');

        // Lead Finder — pull businesses from a free map source (OpenStreetMap),
        // extract WhatsApp/phone + email, add to Contacts / launch a campaign.
        Route::middleware(['workspace.role:manager', 'plan:access_lead_finder'])
            ->prefix('lead-finder')->name('lead-finder.')->group(function () {
                $lf = \App\Http\Controllers\LeadFinderController::class;
                Route::get('/',                 [$lf, 'index'])->name('index');
                Route::post('/search',          [$lf, 'search'])->middleware('throttle:30,1')->name('search');
                Route::post('/key',             [$lf, 'saveKey'])->name('key.save');
                Route::delete('/key',           [$lf, 'removeKey'])->name('key.remove');
                Route::post('/{lead}/contact',  [$lf, 'addContact'])->whereNumber('lead')->name('contact');
                Route::post('/{lead}/enrich',   [$lf, 'enrich'])->whereNumber('lead')->middleware('throttle:40,1')->name('enrich');
                Route::post('/bulk-add',        [$lf, 'bulkAdd'])->name('bulk-add');
        ...
        // ── Company / Organization CRM (Phase 1) ────────────────────────
        // B2B layer over contacts + deals: list, 360 detail (contacts + deals
        // + won-revenue rollup), CRUD, and a JSON search for pickers/copilot.
        Route::middleware('plan:access_sales_pipeline')->prefix('companies')->name('companies.')->group(function () {
            $c = \App\Http\Controllers\CompaniesController::class;
            Route::get('/',        [$c, 'index'])->name('index');
        ...
        // ── AI-CRM Phase 3 — first-class tasks ──────────────────────────
        Route::middleware('plan:access_sales_pipeline')->prefix('tasks')->name('tasks.')->group(function () {
            $tk = \App\Http\Controllers\TasksController::class;
            Route::get('/',                [$tk, 'index'])->name('index');
            Route::post('/',               [$tk, 'store'])->name('store');
            Route::post('/{id}/complete',  [$tk, 'complete'])->whereNumber('id')->name('complete');
            Route::patch('/{id}',          [$tk, 'update'])->whereNumber('id')->name('update');
            Route::delete('/{id}',         [$tk, 'destroy'])->whereNumber('id')->name('destroy');
        });

        // ── AI-CRM Phase 6 — Projects (post-sale delivery tracking) ─────
        Route::middleware('plan:access_sales_pipeline')->prefix('projects')->name('projects.')->group(function () {
            $pj = \App\Http\Controllers\ProjectsController::class;
            Route::get('/',        [$pj, 'index'])->name('index');
            Route::post('/',       [$pj, 'store'])->name('store');
            Route::patch('/{id}',  [$pj, 'update'])->whereNumber('id')->name('update');
            Route::delete('/{id}', [$pj, 'destroy'])->whereNumber('id')->name('destroy');
        });

        // ── AI-CRM Phase 7 — Proposals + Estimates (unified SalesDoc engine) ──
        foreach (['proposals' => 'proposals', 'estimates' => 'estimates'] as $prefix => $name) {
            Route::middleware('plan:access_sales_pipeline')->prefix($prefix)->name($name . '.')->group(function () {
                $sd = \App\Http\Controllers\SalesDocsController::class;
                Route::get('/',                [$sd, 'index'])->name('index');
                Route::post('/',               [$sd, 'store'])->name('store');
                Route::patch('/{id}',          [$sd, 'update'])->whereNumber('id')->name('update');
                Route::post('/{id}/convert',   [$sd, 'convert'])->whereNumber('id')->name('convert');
                Route::post('/{id}/send',      [$sd, 'sendWhatsApp'])->whereNumber('id')->name('send');
                Route::delete('/{id}',         [$sd, 'destroy'])->whereNumber('id')->name('destroy');
            });
        }

        // ── AI-CRM Phase 8 — unified Calendar (aggregates all dated CRM objects) ──
        Route::middleware('plan:access_sales_pipeline')
            ->get('/calendar', [\App\Http\Controllers\CalendarController::class, 'index'])
            ->name('calendar.index');

        // ── AI-CRM Phase 4 — unified CRM dashboard + revenue reports + export ──
        Route::middleware('plan:access_sales_pipeline')->prefix('crm')->name('crm.')->group(function () {
            $crm = \App\Http\Controllers\CrmController::class;
            Route::get('/',             [$crm, 'dashboard'])->name('dashboard');
            Route::get('/guide',        [$crm, 'guide'])->name('guide');
            Route::get('/revenue',      [$crm, 'revenue'])->name('revenue');
            Route::get('/revenue.csv',  [$crm, 'revenueCsv'])->name('revenue.csv');
            Route::get('/revenue.pdf',  [$crm, 'revenuePdf'])->name('revenue.pdf');
            // Phase 5 — generate a Client Brief deck for a contact/company/deal.
            Route::post('/briefs',      [\App\Http\Controllers\BriefsController::class, 'store'])->name('briefs.store');
        });

        // ── AI CRM Copilot ──────────────────────────────────────────────
        // Chat with an AI that runs the CRM by natural language (find/create
        // contacts + deals, move stages, sales reports) via tool-calling.
        // Same service backs the WhatsApp staff channel. Plan-gated as an AI
        // feature; write tools use confirm-before-act + are audit-logged.
        Route::middleware(['workspace.role:manager', 'plan:access_ai_agents'])
            ->prefix('ai-crm')->name('ai-crm.')->group(function () {
                $c = \App\Http\Controllers\AiCrmController::class;
                Route::get('/',        [$c, 'index'])->name('index');
                Route::post('/message', [$c, 'message'])->middleware('throttle:30,1')->name('message');
                Route::post('/reset',  [$c, 'reset'])->name('reset');
                // WhatsApp staff-channel toggle — admins only.
                Route::post('/settings', [$c, 'settings'])->middleware('workspace.role:admin')->name('settings');
            });

        Route::get('/team-inbox',    [\App\Http\Controllers\TeamInboxController::class, 'index'])->name('team-inbox');
        // Pull the Coexistence 6-month phone-app chat history into the inbox.
        // Manager+ gated (bulk import, not an everyday agent action).
        Route::post('/team-inbox/sync-history', [\App\Http\Controllers\TeamInboxController::class, 'syncHistory'])
            ->middleware('workspace.role:manager')->name('team-inbox.sync-history');
        Route::get('/team-inbox/kanban', [\App\Http\Controllers\TeamInboxController::class, 'kanban'])
            ->middleware('plan:access_kanban_view')->name('team-inbox.kanban');
        // Analytics pages — manager+ gated (uses same workspace.role
        // middleware as /analytics so agents/viewers don't see KPIs) + plan-gated.
        Route::get('/team-inbox/analytics/team',      [\App\Http\Controllers\TeamInboxController::class, 'teamAnalyticsPage'])
            ->middleware(['workspace.role:manager', 'plan:access_team_performance'])->name('team-inbox.analytics.team');
        Route::get('/team-inbox/analytics/ai-agents', [\App\Http\Controllers\TeamInboxController::class, 'aiAnalyticsPage'])
            ->middleware(['workspace.role:manager', 'plan:access_team_performance'])->name('team-inbox.analytics.ai');
        ...
        // SLA policies — a BLADE-ONLY settings page (no JS build). The card in
        // the inbox settings used to be a dead link to /team-inbox; it now opens
        // this page. All actions are plain <form> POSTs that redirect back, so
        // the compiled bundle is untouched. Manager+ gated, like analytics.
        Route::get   ('/team-inbox/sla',      [\App\Http\Controllers\TeamInboxController::class, 'slaPage'])
            ->middleware(['workspace.role:manager', 'plan:access_sla_policies'])->name('team-inbox.sla');
        Route::post  ('/team-inbox/sla',      [\App\Http\Controllers\TeamInboxController::class, 'slaWebStore'])
            ->middleware('workspace.role:manager')->name('team-inbox.sla.store');
        Route::put   ('/team-inbox/sla/{id}', [\App\Http\Controllers\TeamInboxController::class, 'slaWebUpdate'])
            ->whereNumber('id')->middleware('workspace.role:manager')->name('team-inbox.sla.update');
        Route::delete('/team-inbox/sla/{id}', [\App\Http\Controllers\TeamInboxController::class, 'slaWebDestroy'])
            ->whereNumber('id')->middleware('workspace.role:manager')->name('team-inbox.sla.destroy');
        ...
        Route::prefix('team-inbox/api')->name('team-inbox.api.')->group(function () {
            ...
            // Team-Inbox PWA Web-Push — the agent's browser fetches the VAPID
            // public key, then stores/removes its push subscription. JS sends the
            // CSRF token header, so these ride the normal web CSRF check.
            $pwa = \App\Http\Controllers\TeamInboxPwaController::class;
            Route::get ('/push/key',         [$pwa, 'pushKey'])->name('push.key');
            Route::post('/push/subscribe',   [$pwa, 'subscribe'])->name('push.subscribe');
            Route::post('/push/unsubscribe', [$pwa, 'unsubscribe'])->name('push.unsubscribe');
            ...
            // Inline contact edit from the thread's contact panel (name / email),
            // update-or-create the Contact for this thread — same as /contacts edit.
            Route::post('/conversations/{id}/save-contact', [$c, 'saveContact'])->whereNumber('id')->name('save-contact');
            Route::post('/conversations/{id}/ai-suggest', [$c, 'aiSuggest'])->whereNumber('id')->name('ai-suggest');
            Route::get ('/ig-flows',                       [$c, 'igFlows'])->name('ig-flows');
            Route::post('/conversations/{id}/run-ig-flow', [$c, 'runIgFlow'])->whereNumber('id')->name('run-ig-flow');
            ...
            // Duplicate-thread cleanup. Manager+ gated like sync-history —
            // it rewrites conversation ownership across the whole workspace,
            // not a per-thread action an agent should reach for.
            Route::get ('/duplicates',       [$c, 'duplicatesScan'])
                ->middleware('workspace.role:manager')->name('duplicates.scan');
            Route::post('/duplicates/merge', [$c, 'duplicatesMerge'])
                ->middleware('workspace.role:manager')->name('duplicates.merge');
            ...
            Route::post('/members/{userId}/reset-password', [$m, 'resetPassword'])->whereNumber('userId')->name('members.reset-password');
        ...
        // Slug constraint MUST match what the admin form allows (alpha_dash:
        // letters incl. UPPERCASE, digits, dashes, underscores). It used to be
        // [a-z0-9-]+, so a bundle whose slug had an uppercase letter or an
        // underscore (e.g. "Bronze" / "bronze_5000") passed validation but its
        // "Get credits" URL matched NO route → the 404 users hit on top-up.
        Route::get ('/checkout/credits/{slug}', [\App\Http\Controllers\CheckoutController::class, 'creditShow'])->where('slug', '[A-Za-z0-9_-]+')->name('checkout.credits.show');
        Route::post('/checkout/credits/{slug}', [\App\Http\Controllers\CheckoutController::class, 'creditProcess'])->where('slug', '[A-Za-z0-9_-]+')->name('checkout.credits.process');
        // "Add any amount" wallet top-up — same gateway-driven checkout, amount rides the query string.
        Route::get ('/checkout/wallet', [\App\Http\Controllers\CheckoutController::class, 'walletTopupShow'])->name('checkout.wallet.show');
        Route::post('/checkout/wallet', [\App\Http\Controllers\CheckoutController::class, 'walletTopupProcess'])->name('checkout.wallet.process');
        ...
        // Wallet statement — full paginated money-in / money-out ledger with
        // filters + CSV export, so the customer can track where every dollar
        // in their wallet came from and went.
