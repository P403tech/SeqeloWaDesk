...
<x-layouts.user :title="__('Integrations')" nav-key="more" page="user-integrations-index">
    ...
    <script>
        ...
        // TikTok Shop connection state — an active shop row for this workspace.
        window.TIKTOK_SHOP_CONNECTED = @json(auth()->user()?->current_workspace_id
                ? \App\Models\TiktokShop::where('workspace_id', auth()->user()->current_workspace_id)->where('status', 'connected')->exists()
                : false);
        window.TIKTOK_SHOP_ENABLED = @json((bool) \App\Models\SystemSetting::get('tiktok_shop_enabled', false));
