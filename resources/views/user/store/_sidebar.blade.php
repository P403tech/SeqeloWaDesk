...
<aside data-keep-rail class="space-y-3 lg:sticky lg:top-6 self-start">
    ...
    <nav class="border border-paper-200 rounded-2xl bg-paper-0 p-2 shadow-card space-y-0.5">
        ...
        {{-- Invoices — auto-generate + WhatsApp-deliver a PDF invoice per order. Rendered IN-store. --}}
        <a href="{{ route('user.store.invoices') }}"
            class="flex items-center gap-2.5 px-3 py-2 rounded-xl text-[12.5px] {{ ($current ?? '') === 'invoices' ? 'bg-wa-mint/40 text-wa-deep font-semibold' : 'hover:bg-paper-50 text-ink-700' }}">
            <svg viewBox="0 0 16 16" class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="1.6">
                <path d="M4 2h6l2 2v10H4V2Z" /><path d="M6 6h4M6 8.5h4M6 11h2.5" />
            </svg>
            {{ __('Invoices') }}
        </a>
