            ...
            <aside data-keep-rail class="space-y-3 lg:sticky lg:top-6 self-start">
                ...
                <nav class="border border-paper-200 rounded-2xl bg-paper-0 p-2 shadow-card space-y-0.5">
                    ...
                    <a data-tab="wallet" href="?tab=wallet" class="acc-tab"><svg viewBox="0 0 16 16" class="w-3.5 h-3.5"
                            ...
                            class="ml-auto text-[10px] font-mono text-wa-deep">{!! \App\Support\FormatSettings::display((int) round(((int) ($authUser->wallet_credits ?? 0)) * \App\Services\MessageCreditRate::minorPerCredit()) / 100) !!}</span></a>
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
                            ...
                            <div>
                                ...
                                <select id="acc-tz" name="timezone" class="w-full">
                                    @php
                                        // Same canonical IANA list the Workspace settings page uses
                                        // (\DateTimeZone::listIdentifiers()). This deliberately does NOT
                                        // come from the browser's Intl.supportedValuesOf() — some browsers
                                        // still list the legacy alias "Asia/Calcutta", which Carbon rejects
                                        // and used to 500 the whole dashboard once it was saved. safe_timezone()
                                        // normalises a legacy stored value (Asia/Calcutta → Asia/Kolkata) so
                                        // the right option is pre-selected.
                                        $current = safe_timezone(old('timezone', $ws->timezone ?? 'Asia/Kolkata'), 'Asia/Kolkata');
                                        try { $accZones = \DateTimeZone::listIdentifiers(); }
                                        catch (\Throwable $e) { $accZones = ['UTC', 'Asia/Kolkata', 'Asia/Dubai', 'Europe/London', 'America/New_York']; }
                                    @endphp
                                    @foreach ($accZones as $z)
                                        <option value="{{ $z }}" @selected($current === $z)>{{ $z }}</option>
                                    @endforeach
                                </select>
    ...
    $orders->where('status', 'paid')->pluck('currency')->unique()->count() === 1
        ...
        'USD');
                ...
                @php
                    ...
                    // Show the balance as MONEY (₹) — the value of the credits at the
                    // current top-up rate — which is what customers expect.
                    $minorPerCredit = \App\Services\MessageCreditRate::minorPerCredit();
                    $moneyOf = fn ($cr) => $minorPerCredit > 0
                        ? \App\Support\FormatSettings::display(((int) $cr * $minorPerCredit) / 100)
                        : null;
                    $balanceMoney = $moneyOf($credits);
                    $perCountryPricing = \App\Services\MessageCreditRate::enabled();
                ...
                <div data-pane="wallet" class="space-y-5 hidden">
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                        <div class="bg-paper-0 border border-wa-green/40 rounded-2xl p-5 shadow-card">
                            <div class="text-[11px] text-ink-600 font-medium">{{ __('Wallet balance') }}</div>
                            <div class="font-serif text-[36px] leading-none mt-2">{{ $balanceMoney ?? number_format($credits) }}</div>
                            <div class="text-[11px] text-wa-deep mt-2">
                                {{ __('Available to spend') }}@unless($perCountryPricing) · ≈ {{ number_format($estMessages) }} {{ $estMessages === 1 ? __('message') : __('messages') }}@endunless
                            </div>
                        </div>
                        <div class="bg-paper-0 border border-paper-200 rounded-2xl p-5 shadow-card">
                            ...
                            <div class="font-serif text-[36px] leading-none mt-2">{{ $moneyOf($spent30d) ?? number_format($spent30d) }}</div>
                            <div class="text-[11px] text-ink-500 mt-2">
                                {{ __('Spent from your wallet') }}
                            </div>
                        </div>
                        <div class="bg-paper-0 border border-accent-amber/40 rounded-2xl p-5 shadow-card">
                            ...
                            <div class="font-serif text-[36px] leading-none mt-2">+{{ $moneyOf($earned30d) ?? number_format($earned30d) }}
                            </div>
                            <div class="text-[11px] text-ink-500 mt-2">{{ __('Added to your wallet in the last 30 days') }}
                            </div>
                        </div>
                    </div>
                    <!-- Add money to your wallet — primary top-up action. Any amount,
                         paid through the same gateways as plans, lands in the wallet. -->
                    @php $curSym = \App\Support\FormatSettings::currencyFor()?->symbol; @endphp
                    <div class="bg-paper-0 border border-wa-green/40 rounded-2xl p-5 shadow-card">
                        <h3 class="font-serif text-[20px] leading-tight">{{ __('Add money to your wallet') }}</h3>
                        <p class="text-[11.5px] text-ink-500 mt-0.5 mb-4">
                            {{ __('Top up any amount — it goes straight to your wallet balance.') }}</p>
                        <form method="GET" action="{{ url('/checkout/wallet') }}"
                            class="flex flex-wrap items-end gap-3">
                            <div class="flex-1 min-w-[180px]">
                                <label for="wallet-amount"
                                    class="text-[11.5px] font-semibold text-ink-700 mb-1.5 block">{{ __('Amount') }}</label>
                                <div class="relative">
                                    @if ($curSym)
                                        <span
                                            class="absolute left-3 top-1/2 -translate-y-1/2 text-[13px] text-ink-500 font-mono">{{ $curSym }}</span>
                                    @endif
                                    <input id="wallet-amount" name="amount" type="number" min="1" step="1" required
                                        placeholder="1000"
                                        class="w-full py-2 border border-paper-200 rounded-lg bg-paper-0 text-[13px] focus:outline-none focus:border-wa-deep focus:ring-4 focus:ring-wa-deep/10 {{ $curSym ? 'pl-7 pr-3' : 'px-3' }}" />
                                </div>
                            </div>
                            <button type="submit"
                                class="px-4 py-2 rounded-full bg-wa-deep hover:bg-wa-teal text-paper-0 text-[12px] font-semibold">{{ __('Add money') }}</button>
                        </form>
                        <div class="flex flex-wrap items-center gap-2 mt-3">
                            <span
                                class="text-[10.5px] font-mono uppercase tracking-[0.14em] text-ink-500">{{ __('Quick add') }}</span>
                            @foreach ([500, 1000, 2000, 5000] as $qa)
                                <a href="{{ url('/checkout/wallet') }}?amount={{ $qa }}"
                                    class="px-3 py-1 rounded-full border border-paper-200 bg-paper-0 hover:border-wa-deep hover:bg-wa-mint/30 text-[11.5px] font-medium">{{ ($curSym ?? '') . number_format($qa) }}</a>
                            @endforeach
                        </div>
                    </div>
                    <div class="flex items-center gap-2">
                        <a href="?tab=affiliate"
                            class="px-4 py-2 rounded-full bg-paper-0 border border-paper-200 hover:border-wa-deep hover:bg-wa-mint/30 text-[12px] font-medium inline-flex items-center gap-2"><svg
                                viewBox="0 0 16 16" class="w-3.5 h-3.5" fill="none" stroke="currentColor"
                                stroke-width="1.7">
                                <path d="M5.5 4.5 3 7l2.5 2.5M10.5 4.5 13 7l-2.5 2.5M7 12l2-8" />
                            </svg>{{ __('Earn via affiliate') }}</a>
                    </div>

                    <!-- How billing works — depends on the workspace billing mode.
                         In pay-per-message / reseller (BSP) mode there is NO plan
                         allowance: every message is charged from the wallet. -->
                    @php
                        ...
                        // Wallet-per-message billing: reseller/BSP customer, or the
                        // platform globally runs pay-per-message. Either way the plan
                        // allowance story does not apply.
                        $walletPerMessage = $ws
                            ? \App\Services\MessageBillingService::payPerMessageFor($ws)
                            : false;
                    @endphp
                    <div class="rounded-2xl bg-wa-mint/40 border border-wa-green/30 p-4 flex items-start gap-3">
                        ...
                        <div class="text-[12px] text-ink-700 leading-relaxed">
                            ...
                            @if ($walletPerMessage)
                                {{ __('Every message you send is charged from your wallet balance at the recipient\'s price — there is no monthly plan allowance.') }}
                                {{ __('Keep your wallet topped up so your messages keep going out. If the balance runs out, sending pauses until you add money.') }}
                            @elseif ($ws && $wsLimit > 0)
                                ...
                                {{ __('When the monthly allowance is used up while your plan is active, extra messages are charged from your wallet balance at each message\'s price.') }}
                                {{ __('After your plan ends, sending pauses until you renew.') }}
                                @unless ($wsActive)
                                    <span
                                        class="block mt-1.5 text-accent-amber font-semibold">{{ __('Your plan has ended — sending is paused until you renew.') }}</span>
                                @endunless
                            @else
                                {{ __('Your plan includes a monthly message allowance. When it is used up while your plan is active, extra messages are charged from your wallet balance at each message\'s price.') }}
                                {{ __('After your plan ends, sending pauses until you renew.') }}
                            @endif
                        </div>
                    </div>

                    <!-- Credit packages — hidden in the money-wallet model (customers
                         top up any amount above instead of buying fixed bundles). -->
                    @if (false && $creditPackages->isNotEmpty())
 ...
 {{ $isFeatured
     ...
     : 'bg-paper-0 border border-paper-200 hover:border-wa-deep hover:bg-wa-mint/30 text-ink-900' }}">
                        ...
                        <div class="px-5 py-4 border-b border-paper-200 flex items-center justify-between gap-3">
                            ...
                            <div class="flex items-center gap-3">
                                @if ($walletLedger->total() > 0)
                                    <span class="text-[10px] font-mono uppercase tracking-[0.14em] text-ink-500 hidden sm:inline">
                                        {{ __('Showing') }} {{ $walletLedger->firstItem() }}–{{ $walletLedger->lastItem() }}
                                        {{ __('of') }} {{ number_format($walletLedger->total()) }}
                                    </span>
                                @endif
                                <a href="{{ route('user.account.wallet.statement') }}"
                                    class="text-[11.5px] font-semibold text-wa-deep hover:underline inline-flex items-center gap-1 whitespace-nowrap">
                                    {{ __('Full statement') }}
                        ...
                        @if ($walletLedger->total() === 0)
                            <div class="px-5 py-10 text-center">
                                @include('user.partials.empty-state', [
                                    'message' =>
                                        'No wallet activity yet. Add money above or earn via the affiliate program to get started.',
                                    ...
                                    @foreach ($walletLedger as $tx)
                                        ...
                                        <tr class="hover:bg-paper-50/60">
                                            ...
                                            <td class="px-4 py-2.5 text-right {{ $amtCls }} font-mono">
                                                {{ $sign }}{{ $moneyOf($abs) ?? number_format($abs) }}</td>
                                            <td class="px-4 py-2.5 text-right font-mono">
                                                {{ $moneyOf((int) $tx->balance_after) ?? number_format((int) $tx->balance_after) }}</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
                ...
                <div data-pane="addons" class="space-y-5 hidden">
                    ...
                    @else
                        {{-- Your active add-ons (purchased + unexpired). --}}
                        @if (($myAddons ?? collect())->isNotEmpty())
                            <div class="bg-paper-0 border border-wa-green/40 rounded-2xl shadow-card overflow-hidden">
                                <div class="px-5 py-4 border-b border-paper-200">
                                    <h3 class="font-serif text-[20px] leading-tight">{{ __('Your add-ons') }}</h3>
                                    <p class="text-[11.5px] text-ink-500 mt-0.5">{{ __('Active packs merged onto your current plan.') }}</p>
                                </div>
                                <div class="divide-y divide-paper-200">
                                    @foreach ($myAddons as $wa)
                                        @php $p = $wa->package; @endphp
                                        @if ($p)
                                            <div class="px-5 py-3 flex items-center justify-between gap-3">
                                                <div class="min-w-0">
                                                    <div class="text-[13px] font-semibold text-ink-900 truncate">{{ $p->pname }}</div>
                                                    <div class="text-[11px] text-ink-500 font-mono">
                                                        @if ($wa->ends_at){{ __('Ends') }} {{ \Illuminate\Support\Carbon::parse($wa->ends_at)->format('M d, Y') }}@else{{ __('Lifetime') }}@endif
                                                    </div>
                                                </div>
                                                <span class="text-[10px] font-mono uppercase tracking-[0.14em] px-2 py-1 rounded-full bg-wa-mint text-wa-deep shrink-0">{{ __('Active') }}</span>
                                            </div>
                                        @endif
                                    @endforeach
                                </div>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 p-5">
                                @foreach ($addons as $a)
                                    @php
                                        ...
                                        $grants = [];
                                        foreach ($addonCatalog['capabilities'] as $k => $l) { if ($a->grantsFeature($k)) $grants[] = $l; }
                                        foreach ($addonCatalog['limits'] as $k => $l) { $d = $a->grantLimit($k); if ($d !== null) $grants[] = ($d === -1 ? __('Unlimited') . ' ' . $l : '+' . $d . ' ' . $l); }
                                        // Owned state — feature packs are one-time (show Active); quantity
                                        // packs (limit boosts) stack, so allow buying again.
                                        $owned     = (int) (($ownedCounts ?? collect())[$a->id] ?? 0);
                                        $stackable = $a->isStackable();
                                    @endphp
                                    <div class="relative rounded-2xl p-5 flex flex-col gap-2 bg-paper-0 border border-paper-200 hover:border-wa-deep/40 transition">
                                        ...
                                        @endif
                                        @if ($owned > 0 && !$stackable)
                                            {{-- One-time feature pack already owned — nothing to gain from re-buying. --}}
                                            <span class="mt-auto inline-flex items-center justify-center gap-1.5 px-3 py-2.5 rounded-full text-[12px] font-semibold bg-wa-mint text-wa-deep">
                                                <svg viewBox="0 0 16 16" class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 8.5l3.5 3.5L13 5" /></svg>
                                                {{ __('Active') }}
                                            </span>
                                        @else
                                            <a href="{{ route('user.checkout.show', $a->id) }}"
                                                ...
                                                {{ $owned > 0 ? __('Buy again') : ($a->cta_label ?: __('Add to plan')) }}
                                                <svg viewBox="0 0 16 16" class="w-3 h-3" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M3 8h10M9 4l4 4-4 4" /></svg>
                                            </a>
                                            @if ($owned > 0 && $stackable)
                                                <div class="text-[10.5px] text-ink-500 text-center mt-1">{{ __('Owned') }} &times;{{ $owned }} · {{ __('buying again adds more') }}</div>
                                            @endif
                                        @endif
                                    </div>
                                @endforeach
                                ...
                                <div class="mt-4 px-4 py-3 rounded-lg bg-paper-50 border border-paper-200">
                                    ...
                                    <ul class="text-[11.5px] text-ink-700 list-disc pl-5 space-y-1">
                                        <li><a href="?tab=wallet"
                                                class="text-wa-deep font-semibold hover:underline">{{ __('Spend your remaining balance') }}</a>
                                            · {!! \App\Support\FormatSettings::display((int) round(((int) ($authUser->wallet_credits ?? 0)) * \App\Services\MessageCreditRate::minorPerCredit()) / 100) !!}
                                            {{ __('will be lost otherwise') }}</li>
