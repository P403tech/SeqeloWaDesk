                    ...
                    <div class="bg-paper-0 border {{ $hook->is_active ? 'border-paper-200' : 'border-paper-200 opacity-75' }} rounded-2xl shadow-card overflow-hidden">
                        ...
                        <div class="px-5 py-4 grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-5 items-start">
                                            ...
                                            @foreach ($hook->events as $ev)
                                                <details class="border border-paper-200 rounded-lg bg-paper-0">
                                                    <summary class="px-3 py-2 cursor-pointer flex items-center gap-2 text-[12px]">
                                                        ...
                                                        @if ($ev->lead_contact_id)
                                                            <a href="{{ url('/contacts') }}?q={{ $ev->lead_contact_id }}" class="ml-auto px-1.5 py-0.5 rounded text-[9.5px] font-mono bg-wa-mint text-wa-deep no-underline">{{ __('lead saved') }}</a>
                                                        @endif
                                                        @if ($ev->forwarded)
                                                            <span class="{{ $ev->lead_contact_id ? '' : 'ml-auto' }} px-1.5 py-0.5 rounded text-[9.5px] font-mono {{ $ev->forward_error ? 'bg-accent-coral/10 text-accent-coral' : 'bg-wa-mint text-wa-deep' }}">
                            ...
                            <aside>
                                ...
                                {{-- Lead capture — turn each received payload into a Contact --}}
                                @php $lc = is_array($hook->lead_config) ? $hook->lead_config : []; @endphp
                                <form method="POST" action="{{ route('user.webhooks.incoming.lead-capture', $hook->id) }}"
                                    class="mt-4 rounded-2xl border border-paper-200 bg-paper-50/40 p-4">
                                    @csrf
                                    <div class="font-mono text-[10px] uppercase tracking-[0.16em] text-ink-500">{{ __('Lead capture') }}</div>
                                    <h3 class="font-serif text-[16px] leading-tight mt-0.5 mb-2">{{ __('Save as a contact') }}</h3>
                                    <label class="flex items-center gap-2 text-[12px] font-semibold text-ink-800 mb-3">
                                        <input type="checkbox" name="lead_enabled" value="1" class="accent-wa-deep" @checked(!empty($lc['enabled']))>
                                        {{ __('Create a contact from each request') }}
                                    </label>

                                    <div class="space-y-2">
                                        <div>
                                            <span class="text-[10px] font-mono uppercase tracking-wide text-ink-500">{{ __('Phone field') }}</span>
                                            <input type="text" name="lead_phone" value="{{ $lc['phone'] ?? '' }}" placeholder="phone"
                                        ...
                                        <div class="grid grid-cols-2 gap-2">
                                            <div>
                                                <span class="text-[10px] font-mono uppercase tracking-wide text-ink-500">{{ __('Name field') }}</span>
                                                <input type="text" name="lead_name" value="{{ $lc['name'] ?? '' }}" placeholder="name"
                                                    class="mt-0.5 w-full px-3 py-1.5 border border-paper-200 rounded-lg bg-white text-[12px] font-mono focus:outline-none focus:border-wa-deep">
                                            </div>
                                            <div>
                                                <span class="text-[10px] font-mono uppercase tracking-wide text-ink-500">{{ __('Email field') }}</span>
                                                <input type="text" name="lead_email" value="{{ $lc['email'] ?? '' }}" placeholder="email"
                                                    class="mt-0.5 w-full px-3 py-1.5 border border-paper-200 rounded-lg bg-white text-[12px] font-mono focus:outline-none focus:border-wa-deep">
                                            </div>
                                        </div>
                                        <div class="grid grid-cols-2 gap-2">
                                            <div>
                                                ...
                                                <input type="text" name="lead_cc" value="{{ $lc['country_code'] ?? '' }}" placeholder="e.g. 91"
                                                    class="mt-0.5 w-full px-3 py-1.5 border border-paper-200 rounded-lg bg-white text-[12px] font-mono focus:outline-none focus:border-wa-deep">
                                            </div>
                                            <div>
                                                <span class="text-[10px] font-mono uppercase tracking-wide text-ink-500">{{ __('Tag') }}</span>
                                                <input type="text" name="lead_tag" value="{{ $lc['tag'] ?? 'Lead' }}" placeholder="Lead"
                                        ...
                                        <div>
                                            <span class="text-[10px] font-mono uppercase tracking-wide text-ink-500">{{ __('Start a flow (optional)') }}</span>
                                            <select name="lead_flow_id"
                                                class="mt-0.5 w-full px-3 py-1.5 border border-paper-200 rounded-lg bg-white text-[12px] focus:outline-none focus:border-wa-deep">
                                                <option value="">{{ __('— none —') }}</option>
                                                @foreach (($flows ?? []) as $f)
                                                    <option value="{{ $f['id'] }}" @selected((int)($lc['flow_id'] ?? 0) === (int)$f['id'])>{{ $f['name'] }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
