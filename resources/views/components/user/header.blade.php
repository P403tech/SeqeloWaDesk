...
$allNavItems = [
    ...
    [
        // Promoted into the default header (takes Meta Ads' old slot).
        'key' => 'team-inbox',
        'tier' => 'admin',
        'href' => url('/team-inbox'),
        'label' => __('Unified Inbox'),
        'icon' => '<rect x="2" y="3" width="12" height="9" rx="1.5"/><path d="M2 6h12M5 9h2.5"/>',
        'sw' => 1.5,
    ],
    [
        // Moved OFF the default header into the /more page. Kept in the
        // catalogue (so an admin can promote it back via menu-order) but
        // `promo` keeps it out of the top bar unless explicitly placed.
        'key' => 'metaads',
        'tier' => 'admin',
        'href' => url('/meta-ads'),
        'label' => __('Meta Ads'),
    ...
    [
        ...
        'href' => url('/devices'),
        'label' => __('Channels'),
        'icon' => '<circle cx="8" cy="8" r="2.2"/><circle cx="3" cy="3.5" r="1.4"/><circle cx="13" cy="3.5" r="1.4"/><circle cx="3" cy="12.5" r="1.4"/><circle cx="13" cy="12.5" r="1.4"/><path d="M4.2 4.6 6.4 6.7M11.8 4.6 9.6 6.7M4.2 11.4 6.4 9.3M11.8 11.4 9.6 9.3"/>',
        'sw' => 1.6,
    ],
    ...
    [
        'key' => 'crm', 'tier' => 'admin', 'href' => url('/crm'), 'label' => __('CRM'),
        'icon' => '<rect x="2" y="2" width="5" height="5" rx="1"/><rect x="9" y="2" width="5" height="5" rx="1"/><rect x="2" y="9" width="5" height="5" rx="1"/><rect x="9" y="9" width="5" height="5" rx="1"/>', 'sw' => 1.3, 'promo' => true, 'feature' => 'access_sales_pipeline',
    ],
    [
    ...
    [
        'key' => 'ai-crm', 'tier' => 'admin', 'href' => url('/ai-crm'), 'label' => __('AI CRM'),
        'icon' => '<rect x="3" y="4" width="10" height="8" rx="2"/><path d="M6 13l2-2h3"/><circle cx="6.2" cy="7.5" r="0.9"/><circle cx="9.8" cy="7.5" r="0.9"/><path d="M8 2v2"/>', 'sw' => 1.4, 'promo' => true, 'feature' => 'access_ai_agents',
    ],
    [
        'key' => 'companies', 'tier' => 'admin', 'href' => url('/companies'), 'label' => __('Companies'),
        'icon' => '<rect x="2.5" y="5" width="5" height="8.5" rx="1"/><rect x="8.5" y="2.5" width="5" height="11" rx="1"/><path d="M10.2 5.5h1.6M10.2 8h1.6M10.2 10.5h1.6M4.4 8h1.4M4.4 10.5h1.4"/>', 'sw' => 1.3, 'promo' => true, 'feature' => 'access_sales_pipeline',
    ],
    [
        'key' => 'payments', 'tier' => 'admin', 'href' => url('/payments'), 'label' => __('Payments'),
        'icon' => '<rect x="2" y="3.5" width="12" height="9" rx="1.5"/><path d="M2 6.5h12M4.5 10h3"/>', 'sw' => 1.3, 'promo' => true, 'feature' => 'auto_invoice',
    ],
    [
        'key' => 'tasks', 'tier' => 'admin', 'href' => url('/tasks'), 'label' => __('Tasks'),
        'icon' => '<path d="M5.5 8l1.8 1.8L11 5.5"/><rect x="2" y="2" width="12" height="12" rx="2.5"/>', 'sw' => 1.4, 'promo' => true, 'feature' => 'access_sales_pipeline',
    ],
    [
        'key' => 'projects', 'tier' => 'admin', 'href' => url('/projects'), 'label' => __('Projects'),
        'icon' => '<path d="M2 4.5h5l1.2 1.5H14v6.5a1 1 0 0 1-1 1H3a1 1 0 0 1-1-1V4.5Z"/>', 'sw' => 1.3, 'promo' => true, 'feature' => 'access_sales_pipeline',
    ],
    [
        'key' => 'proposals', 'tier' => 'admin', 'href' => url('/proposals'), 'label' => __('Proposals'),
        'icon' => '<rect x="3" y="1.5" width="10" height="13" rx="1.5"/><path d="M5.5 5h5M5.5 8h5M5.5 11h3"/>', 'sw' => 1.3, 'promo' => true, 'feature' => 'access_sales_pipeline',
    ],
    [
        'key' => 'estimates', 'tier' => 'admin', 'href' => url('/estimates'), 'label' => __('Estimates'),
        'icon' => '<rect x="2.5" y="2" width="11" height="12" rx="1.5"/><path d="M5 5.5h6M5 8h6M8.5 10.5h2.5"/>', 'sw' => 1.3, 'promo' => true, 'feature' => 'access_sales_pipeline',
    ],
    [
        'key' => 'calendar', 'tier' => 'admin', 'href' => url('/calendar'), 'label' => __('Calendar'),
        'icon' => '<rect x="2" y="3" width="12" height="11" rx="1.5"/><path d="M2 6h12M5.5 1.5v3M10.5 1.5v3"/>', 'sw' => 1.3, 'promo' => true, 'feature' => 'access_sales_pipeline',
    ],
    [
...
$logoName = $wsLogo ? ($__bw->name ?: $brandName) : $brandName;
        ...
        {{-- (Removed) The "Switch to Instagram" header icon used to render whenever
             the Instagram add-on was installed. It's gone — Instagram is used
             inside WaDesk (Unified Inbox, dashboard card, flows), so there's no
             separate app to switch to, and the icon in the header was unwanted. --}}
        ...
        @php
            ...
            // Role gate — an invited AGENT / VIEWER / MANAGER must not create new
            // workspaces (matches the workspace.role:admin gate on the route), so
            // only an owner/admin of the current workspace (or a platform super
            // admin) sees the "Create new workspace" link at all.
 ...
 header / pushing the nav off its row. --}}
                @php
                    $wsColor = workspace_brand_color($currentWs, '');
                            ...
                            <form method="POST" action="{{ route('workspaces.switch', $w->id) }}" class="block">
                                ...
                                <button type="submit"
                                    ...
                                    <span
                                        ...
                                        style="background:{{ workspace_brand_color($w) }};">{{ strtoupper(substr($w->name, 0, 2)) }}</span>
...
<script>
    (function() {
        ...
        function renderItems(items) {
            ...
            notifList.innerHTML = items.map((n) => {
                ...
                return `
 <a href="${escapeHtml(href)}" data-notif-id="${n.id}" data-unread="${n.unread ? 1 : 0}" class="block px-4 py-3 hover:bg-paper-50 transition flex items-start gap-2.5">
 ${dot}
 ...
 <div class="text-[10.5px] text-ink-500 font-mono mt-1">${escapeHtml(n.time_ago)}</div>
        ...
        // Mark a SINGLE notification read when its row is clicked. The row is an
        // <a> that navigates on click, so the POST uses keepalive (survives the
        // navigation) and we optimistically drop the badge by one when the clicked
        // item was unread. Previously only "Mark all read" cleared the count — a
        // single click navigated away without ever hitting the read endpoint.
        notifList?.addEventListener('click', (e) => {
            const a = e.target.closest('a[data-notif-id]');
            if (!a) return;
            const id = a.dataset.notifId;
            if (!id) return;
            const wasUnread = a.dataset.unread === '1';
            ...
            if (wasUnread) {
                a.dataset.unread = '0';
                const cur = (notifBadge && !notifBadge.classList.contains('hidden')) ?
                    (parseInt(notifBadge.textContent, 10) || 0) : 0;
