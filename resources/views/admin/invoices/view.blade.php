...
<x-layouts.admin :title="__('Invoice') . ' ' . $invNo" admin-key="invoices" page="invoices-view">
    {{-- Print isolation: window.print() prints inside the admin layout (sidebar +
         chrome). Hide everything, then reveal ONLY the invoice document so Print /
         Download-PDF produce a clean, single invoice — not the whole webpage. --}}
    <style>
        @media print {
                ...
                <div class="flex flex-col sm:flex-row items-start justify-between gap-6">
                    <div>
                        <div class="flex items-center gap-2.5">
                            @if (!empty($billing['logo']))
                                {{-- Uploaded invoice logo (from /admin/checkout-settings).
                                     It already carries the brand, so we don't repeat the
                                     company name as a wordmark next to it. --}}
                                <img src="{{ $billing['logo'] }}" alt="{{ $billing['company'] }}"
                                    class="max-h-14 max-w-[240px] object-contain" />
                            @else
                                <span
