                            ...
                            <li class="px-5 py-4 flex items-start gap-4">
                                ...
                                <div class="min-w-0 flex-1">
                                    ...
                                    <div class="mt-1.5 text-[11px] text-ink-500 leading-relaxed">
                                        Set all three topics — <span
                                            class="font-mono text-ink-700">{{ __('customers/data_request') }}</span>,
                                        <span class="font-mono text-ink-700">{{ __('customers/redact') }}</span>,
                                        <span class="font-mono text-ink-700">{{ __('shop/redact') }}</span> — to this
                                        one fixed URL (unlike the per-store order webhook, it carries no secret so
                                        Shopify's automated check can reach it before any store installs).
                                        Each delivery is HMAC-SHA256 signed with your Client Secret — we verify
                                        <span class="font-mono">{{ __('X-Shopify-Hmac-Sha256') }}</span> and reject
                                        mismatches with <span class="font-mono">401</span>.
                                    </div>
                                </div>
                            </li>
