        ...
        <div class="flex flex-wrap items-end justify-between gap-4">
            <div>
                <div class="font-mono text-[10px] uppercase tracking-[0.18em] text-ink-500 mb-2">
                    {{ __('Message pricing & wallet') }}</div>
                <h1 class="font-serif font-normal tracking-[-0.01em] text-[28px] sm:text-[40px] leading-[1.0]">{{ __('What you') }}
                    <span class="italic text-wa-deep">{{ __('charge') }}</span> {{ __('per message') }}.</h1>
                <p class="text-[13px] text-ink-600 mt-2 max-w-2xl">
                    {{ __('Set the price your customers pay for each WhatsApp message — one price per destination country, so a message to the US can cost more than one to India (that is exactly how WhatsApp bills you). Scroll to') }}
                    <a href="#message-pricing" class="text-wa-deep font-semibold underline decoration-dotted underline-offset-2">{{ __('Per-message pricing') }}</a>
                    {{ __('to set it. The affiliate & top-up settings below are optional.') }}
                </p>
            </div>
...
$defaultSym = $defaultCur?->symbol ?? ($defaultCur?->code ?? '');
        ...
        <section id="message-pricing" class="mt-8 scroll-mt-20 bg-paper-0 border border-paper-200 rounded-2xl p-6 shadow-card">
            <form method="POST" action="{{ route('admin.settings.message-rates.update') }}">
                ...
                <div class="flex flex-wrap items-start justify-between gap-4 mb-4">
                    <div>
                        ...
                        <p class="text-[12.5px] text-ink-500 mt-1 max-w-2xl">
                            {{ __('Charge fairly per message by recipient country and category — the way Meta prices (a US marketing message costs ~12× one to India; service-window replies are free). Add the Meta wholesale cost to see your margin. Leave OFF to keep the single flat rate above.') }}
                        </p>
                    </div>
                ...
                {{-- Billing mode — the big switch: plan quota vs pure prepaid wallet --}}
                <div class="rounded-xl border-2 {{ ($payPerMessage ?? false) ? 'border-wa-deep bg-wa-deep/5' : 'border-paper-200 bg-paper-50/50' }} p-4 mb-4">
                    <label class="flex items-start gap-3 cursor-pointer">
                        <input type="checkbox" name="bsp_pay_per_message" value="1" class="peer sr-only" @checked($payPerMessage ?? false)>
                        ...
                        <span>
                            <span class="font-serif text-[16px] text-ink-900">{{ __('Pay-per-message mode (prepaid wallet)') }}</span>
                            <span class="block text-[12.5px] text-ink-600 mt-1 leading-relaxed">
                                {{ __('ON: the plan\'s monthly message limit is IGNORED — EVERY message is billed to the customer\'s wallet, and the wallet balance is the only thing that lets them send (prepaid pay-per-message model). An expired plan no longer blocks sending as long as the wallet has funds.') }}
                            </span>
                            <span class="block text-[12.5px] text-ink-500 mt-1.5">
                                {{ __('OFF: classic model — the plan gives a free monthly message quota, and the wallet only pays for messages sent OVER that quota.') }}
                            </span>
                        </span>
                    </label>
                </div>

                {{-- Sync from Meta — fills the Meta-cost column for every country at once. --}}
                <div class="flex flex-wrap items-center gap-3 mb-4 p-3 rounded-xl border border-paper-200 bg-paper-50/60">
                    <div class="text-[12px] text-ink-600 flex-1 min-w-[220px]">
                        <span class="font-semibold text-ink-800">{{ __('Cross-border note:') }}</span>
                        {{ __('WhatsApp bills by the RECIPIENT country. A message from India to a US number costs the US rate — so each destination country needs its own row.') }}
                    </div>
                    <button type="submit" formaction="{{ route('admin.settings.message-rates.sync-meta') }}"
                        class="inline-flex items-center gap-1.5 px-4 py-2 rounded-full bg-wa-deep hover:bg-wa-teal text-paper-0 text-[12px] font-semibold"
                        onclick="return confirm('{{ __('Load Meta wholesale costs for every country (converted to :cur)? Your customer prices are not touched.', ['cur' => $defaultSym ?: '₹']) }}')">
                        <svg viewBox="0 0 16 16" class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="1.6"><path d="M13.5 8a5.5 5.5 0 1 1-1.6-3.9M13.5 2v3h-3"/></svg>
                        {{ __('Sync Meta costs') }}
                    </button>
                </div>

                {{-- Import your OWN official rate card (from WhatsApp Manager) — the exact,
                     commercial-safe source. Inputs post to the separate #rateImportForm below. --}}
                <div class="flex flex-wrap items-center gap-3 mb-4 p-3 rounded-xl border border-paper-200 bg-paper-50/60">
                    <div class="text-[12px] text-ink-600 flex-1 min-w-[220px]">
                        <span class="font-semibold text-ink-800">{{ __('Official source:') }}</span>
                        {{ __('Download the official "rates by country" CSV (in your currency) from business.whatsapp.com → Pricing, or WhatsApp Manager, and upload it here. Meta\'s own wide CSV (a Marketing / Utility / Authentication column per country) works as-is, and is converted to your currency.') }}
                    </div>
                    <input type="file" name="file" accept=".csv,text/csv" required form="rateImportForm"
                        class="text-[11px] file:mr-2 file:px-3 file:py-1.5 file:rounded-full file:border-0 file:bg-paper-200 file:text-ink-800 file:text-[11px] file:font-semibold">
                    <button type="submit" form="rateImportForm"
                        class="inline-flex items-center gap-1.5 px-4 py-2 rounded-full hairline border border-paper-200 bg-paper-0 hover:bg-paper-50 text-[12px] font-semibold">
                        {{ __('Import my rate card') }}
                    </button>
                </div>
                ...
                <div class="overflow-x-auto rounded-xl border border-paper-200"
                     data-wallet-rates data-money-per-credit="{{ $moneyPerCredit }}" data-currency="{{ $curSym }}">
                    <table class="w-full text-[12.5px]">
                        <thead class="bg-paper-50 text-ink-500 text-[11px] uppercase tracking-wide">
                            <tr>
                                <th class="text-left font-semibold px-3 py-2">{{ __('Country') }}</th>
                                <th class="text-left font-semibold px-3 py-2">{{ __('Category') }}</th>
                                <th class="text-left font-semibold px-3 py-2">{{ __('You charge') }}<br><span class="normal-case font-normal text-[10px] text-ink-400">{{ $curSym }} {{ __('/ msg') }}</span></th>
                                <th class="text-left font-semibold px-3 py-2">{{ __('Meta cost') }}<br><span class="normal-case font-normal text-[10px] text-ink-400">{{ $curSym }} {{ __('/ msg') }}</span></th>
                                <th class="text-left font-semibold px-3 py-2">{{ __('Your margin') }}</th>
                        ...
                        <tbody class="divide-y divide-paper-100">
                            @foreach (($messageRates ?? collect()) as $r)
                                <tr data-rate-row>
                                    ...
                                    <td class="px-3 py-2 whitespace-nowrap">
                                        <span class="text-ink-400 text-[11px]">{{ $curSym }}</span>
                                        <input type="number" name="credits[]" min="0" step="0.01" data-credits
                                            value="{{ number_format(($r->credits ?? 0) / 100, 2, '.', '') }}" placeholder="0.00"
                                            class="w-24 px-2 py-1.5 border border-paper-200 rounded-lg bg-paper-0 font-mono">
                                    </td>
                                    <td class="px-3 py-2 whitespace-nowrap">
                                        <span class="text-ink-400 text-[11px]">{{ $curSym }}</span>
                                        <input type="number" name="meta_cost[]" min="0" step="0.0001" data-metacost
                                            value="{{ $r->meta_cost_minor !== null ? number_format($r->meta_cost_minor / 100, 4, '.', '') : '' }}"
                                            placeholder="—" class="w-24 px-2 py-1.5 border border-paper-200 rounded-lg bg-paper-0 font-mono">
                                        <input type="hidden" name="currency[]" value="{{ $r->currency ?: $curSym }}">
                                    </td>
                                    <td class="px-3 py-2 text-[12px] font-mono" data-margin>—</td>
                            ...
                            @endforeach
                            @for ($i = 0; $i < 2; $i++)
                                <tr class="bg-paper-50/40" data-rate-row>
                                    <td class="px-3 py-2">
                                    ...
                                    <td class="px-3 py-2 whitespace-nowrap">
                                        <span class="text-ink-400 text-[11px]">{{ $curSym }}</span>
                                        <input type="number" name="credits[]" min="0" step="0.01" placeholder="0.00" data-credits
                                            class="w-24 px-2 py-1.5 border border-paper-200 rounded-lg bg-paper-0 font-mono">
                                    </td>
                                    <td class="px-3 py-2 whitespace-nowrap">
                                        <span class="text-ink-400 text-[11px]">{{ $curSym }}</span>
                                        <input type="number" name="meta_cost[]" min="0" step="0.0001" placeholder="—" data-metacost
                                            class="w-24 px-2 py-1.5 border border-paper-200 rounded-lg bg-paper-0 font-mono">
                                        <input type="hidden" name="currency[]" value="{{ $curSym }}">
                                    </td>
                                    <td class="px-3 py-2 text-[12px] font-mono" data-margin>—</td>
                                    <td></td>
                                </tr>
        ...
        {{-- Standalone upload target for "Import my rate card" (file inputs above
             reference this by form="rateImportForm"; kept outside the editor form
             because a multipart upload can't nest inside another form). --}}
        <form id="rateImportForm" method="POST" action="{{ route('admin.settings.message-rates.import') }}" enctype="multipart/form-data" class="hidden">
            @csrf
        </form>
    </main>
</x-layouts.admin>
