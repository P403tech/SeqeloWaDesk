<x-layouts.admin :title="__('System Message Setting')" admin-key="wadesk-message" page="settings-wadesk-message">
 ...
 when WABA is the active engine. --}}
                @if ($wabaEnabled)
                    ...
                    {{-- Facebook Pages channel — same Meta app, one toggle. When on,
                         workspaces with the Facebook plan feature can connect their
                         Facebook account (all its Pages) from /devices and manage
                         posts, comments & Messenger in the unified inbox. --}}
                    ...
                    {{-- SMS channel — Twilio / MSG91. Just an on/off here; each workspace
                         connects its own number at /devices, reusing the Twilio keys it
                         already pasted for WhatsApp. Inbound texts land in the same team
                         inbox; campaigns get an SMS sender option. --}}
                    <section class="bg-paper-0 border border-paper-200 rounded-2xl shadow-card overflow-hidden">
                        <div class="px-5 py-4 flex items-center justify-between gap-4">
                            <div class="flex items-start gap-3 min-w-0">
                    ...
                    {{-- TikTok channel — a SEPARATE "TikTok for Developers" app (Login Kit
                         + Display + Content Posting), so it needs its own client_key /
                         client_secret (not the Meta app). Turn it on and workspaces with
                         the TikTok plan feature connect their account from Channels. --}}
                    <section class="bg-paper-0 border border-paper-200 rounded-2xl shadow-card overflow-hidden">
                        <div class="px-5 py-4 flex items-center justify-between gap-4">
                            <div class="flex items-start gap-3 min-w-0">
                    ...
                    {{-- Telegram channel — Bot API (plain HTTPS webhook, no OAuth/review):
                         a workspace pastes a @BotFather token and it works. The API
                         id/hash below are OPTIONAL and only power the in-app bot maker
                         (log a user account in over MTProto to drive @BotFather). Turn
                         the channel on and workspaces with the Telegram plan feature
                         connect bots from Channels. --}}
                    <section class="bg-paper-0 border border-paper-200 rounded-2xl shadow-card overflow-hidden">
                        <div class="px-5 py-4 flex items-center justify-between gap-4">
                            <div class="flex items-start gap-3 min-w-0">
                    ...
                    {{-- Registration WhatsApp OTP verification. Sends a code to the
 signup mobile from an admin-selected device on ANY engine:
 Unofficial (text), WABA (approved auth template), Twilio (text). --}}
                    <section class="bg-paper-0 border border-wa-green/40 rounded-2xl shadow-card overflow-hidden">
                        <div class="px-5 py-4 border-b border-paper-200 flex items-center justify-between gap-4">
                            <div>
                                <div class="font-mono text-[10px] uppercase tracking-[0.16em] text-ink-500">
 ...
 --}}
                ...
                {{-- Campaign auto-end (expiry). Fields use form="wadesk-providers-form"
                     so they save with the main Save button at the top of the page. --}}
                <section class="bg-paper-0 border border-paper-200 rounded-2xl shadow-card overflow-hidden">
                    <div class="px-5 py-4 border-b border-paper-200">
