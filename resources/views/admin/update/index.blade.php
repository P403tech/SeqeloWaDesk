<x-layouts.admin :title="__('Updater')" admin-key="settings" page="settings-updater">
    ...
    <main class="px-4 sm:px-7 py-7 space-y-5" id="wd-updater">
        ...
        {{-- Licence re-check is skipped on a live install so each update
             does not send the operator back through the purchase wizard. --}}
        <section class="bg-paper-0 border border-paper-200 rounded-2xl shadow-card overflow-hidden">
            <div class="px-5 py-4 flex items-center gap-3">
                <div data-badge="0" class="w-8 h-8 rounded-full bg-wa-deep text-paper-0 flex items-center justify-center text-[13px] font-bold shrink-0">
                    <svg viewBox="0 0 16 16" class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2"><path d="m3 8 3.5 3.5L13 5"/></svg>
                </div>
                <div>
                    <div class="font-mono text-[10px] uppercase tracking-[0.16em] text-ink-500">{{ __('step 1 · licence') }}</div>
                    <h2 class="font-serif text-[22px] leading-tight mt-0.5">{{ __('Purchase already verified') }}</h2>
                    <p class="text-[12.5px] text-ink-600 mt-1">{{ __('This install is live. You can run backup → upload → apply without re-entering a purchase code after each update.') }}</p>
                </div>
            </div>
        </section>
    ...
    <script>
        window.WD_UPDATER = {
            ...
            urls: {
                ...
                applyStatus: '{{ route('admin.update.apply-status') }}',
