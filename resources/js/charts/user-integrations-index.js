export default function init() {
    // Brand SVG tiles — official-style logos rendered from path data.
      ...
      // Zapier — orange rounded square with the signature burst
      const ZAPIER_SVG = `
        <rect x="4" y="4" width="24" height="24" rx="6" fill="#FF4A00"/>
        <g stroke="#fff" stroke-width="2.4" stroke-linecap="round">
          <path d="M16 8v16"/>
          <path d="M8 16h16"/>
          <path d="M10.3 10.3l11.4 11.4"/>
          <path d="M21.7 10.3 10.3 21.7"/>
        </g>`;

      // TikTok Shop — black rounded tile with the TikTok note glyph.
      const TIKTOK_SHOP_SVG = `
        <path fill="#010101" d="M4 4h24v24H4z" rx="6"/>
        <path fill="#fff" d="M20.6 10.3a4.9 4.9 0 0 1-3-4.3h-3.5v14.1a3 3 0 1 1-3-3c.3 0 .6 0 .9.1v-3.6a6.5 6.5 0 1 0 5.6 6.4V9.5a8.3 8.3 0 0 0 4.9 1.6V7.6a4.9 4.9 0 0 1-1.9-.5z"/>`;

      const AI_CRM_SVG = `
        <g fill="none" stroke="#0b3d2e" stroke-width="2" stroke-linecap="round">
          <circle cx="16" cy="16" r="5"/><path d="M16 5v3M16 24v3M5 16h3M24 16h3M8.2 8.2l2 2M21.8 21.8l-2-2M23.8 8.2l-2 2M8.2 23.8l2-2"/></g>`;

      const APP = (window.WADESK_BRAND && window.WADESK_BRAND.appName) || 'WaDesk';
      const APPS = [
        { id:'ai-crm',      name:'AI CRM',           cat:'crm',          desc:'Your whole CRM — estimates, proposals, invoices, payments, projects, calendar and deals — run it by chat with the AI copilot. Includes a how-to guide.', tile:brandTile('#E7F5EE', AI_CRM_SVG),  connected:false, official:false },
        ...
        { id:'tiktok-shop', name:'TikTok Shop',      cat:'ecom',         desc:'Handle TikTok Shop buyer messages in the unified inbox and pull order context into chats. Partner-approval + region required.', tile:brandTile('#111111', TIKTOK_SHOP_SVG), connected: !!window.TIKTOK_SHOP_CONNECTED, official:true },
      ...
      function render() {
        ...
        list.forEach(a => {
          ...
          // anything else.
          const setupHref = a.id === 'ai-crm'     ? '/crm/guide'
                          : a.id === 'gsheets'    ? '/sheets-addon'
                          : a.id === 'wa-catalog' ? '/catalog'
                          : a.id === 'tiktok-shop'? '/tiktok-shop'
