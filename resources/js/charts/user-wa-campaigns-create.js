...
export default function init() {
    ...
    (function wireEngineGate() {
        ...
        const sync = () => {
            ...
            // The selected sender's account id (wa_provider_configs.id for WABA).
            // WABA templates belong to a SPECIFIC account (provider_config_id),
            // so a workspace with two WABA accounts must only see the picked
            // account's templates — not both accounts' mixed together.
            const accountId = String(sel.value || '').split(':')[1] || '';
            // True when a template option is allowed for the picked WABA account.
            // Non-WABA engines (Baileys/Twilio) have no per-account template
            // scoping, and a template with no stored account (legacy/null) is
            // left visible rather than hidden.
            const wabaAccountOk = (o) => engine !== 'waba' || !accountId
                || !o.dataset.providerConfig || String(o.dataset.providerConfig) === accountId;
            const limited = engine === 'twilio' || engine === 'waba';

            // Official WABA can ONLY send approved Meta templates — a custom
            // free-form broadcast and per-recipient flows don't apply on a WABA
            // number. Hide those two campaign-type cards for a WABA sender and
            // force "Use template" so the wizard can't sit on a dead option.
            const isWaba = engine === 'waba';
            const cardFor = (val) =>
                document.querySelector("input[name='campaign_type'][value='" + val + "']")?.closest('[data-type]');
            ['text', 'flow'].forEach((val) => {
                const card = cardFor(val);
                if (card) card.classList.toggle('hidden', isWaba);
            });
            if (isWaba) {
                const checked = document.querySelector("input[name='campaign_type']:checked");
                if (!checked || checked.value === 'text' || checked.value === 'flow') {
                    const tpl = document.querySelector("input[name='campaign_type'][value='template']");
                    if (tpl && !tpl.checked) {
                        tpl.checked = true;
            ...
            if (tplSel && tplOpts.length) {
                let usableVisible = 0;
                tplOpts.forEach((o) => {
                    const match = (!engine || o.dataset.channel === engine) && wabaAccountOk(o);
                    o.hidden = !match;
                    if (match && !o.disabled) usableVisible++;
                });
                const cur = tplSel.selectedOptions[0];
                // Clear the selection if it no longer fits the picked engine OR
                // (for WABA) the picked account, so a stale cross-account
                // template can't be submitted.
                if (cur && ((cur.dataset.channel && engine && cur.dataset.channel !== engine) || !wabaAccountOk(cur))) {
                    tplSel.value = '';
                }
                if (tplEmpty) {
            ...
            if (tplSelB && tplOptsB.length) {
                ...
                if (curB && ((curB.dataset.channel && engine && curB.dataset.channel !== engine) || !wabaAccountOk(curB))) {
                    tplSelB.value = '';
                }
            }
    ...
    const total = 5;
        ...
        try {
          ...
          if (btnsWrap) {
              btnsWrap.innerHTML = btns.length
                  ...
                  : '';
              // The element starts hidden (and updatePreview re-hides it in
              // custom mode). Filling innerHTML without clearing `hidden`
              // meant a template's CTAs were rendered into an invisible box —
              // the preview looked like a plain text message.
              btnsWrap.classList.toggle('hidden', btns.length === 0);
          }
      ...
      // The send-time mapping panel writes into a hidden field, which fires
      // no input event the form can see — it signals us directly instead so
      // the phone preview reflects typed overrides immediately.
      document.addEventListener("tlm:changed", updatePreview);
