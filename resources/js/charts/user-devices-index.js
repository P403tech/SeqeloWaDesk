...
/* ---- Facebook connect modal (Channels page) ----
 * Opened by any [data-facebook-connect] trigger (the chooser card). Holds the
 * "Continue with Facebook" account login + the manual Page-token form. Mirrors
 * the Instagram connect wiring (which lives in app.js) so Facebook has the same
 * "connect a channel" affordance. No-op when the modal isn't on the page.
 */
function wireFacebookConnect() {
    const modal = $('facebook-connect-modal');
    if (!modal) return;
    const open  = () => {
        // Close the channel chooser if it's open underneath (same as Instagram).
    ...
    document.querySelectorAll('[data-facebook-connect]').forEach((b) =>
        b.addEventListener('click', (e) => { e.preventDefault(); open(); }));
    modal.querySelectorAll('[data-fb-modal-close]').forEach((b) =>
        b.addEventListener('click', close));
    modal.addEventListener('click', (e) => { if (e.target === modal) close(); });
    document.addEventListener('keydown', (e) => {
        if (e.key === 'Escape' && !modal.classList.contains('hidden')) close();
    ...
    // Reveal / hide the manual Page-token form.
    const toggle = modal.querySelector('[data-fb-manual-toggle]');
    const form   = $('fb-manual-form');
    if (toggle && form) {
        toggle.addEventListener('click', () => form.classList.toggle('hidden'));
    }

    // If a manual connect failed server-side, re-open with the error shown
    // (the blade stamps these data-attrs when $errors->has('facebook')).
    if (modal.dataset.fbOpenOnLoad === '1') {
        open();
        if (form && modal.dataset.fbManualOpen === '1') form.classList.remove('hidden');
    }
}
...
export default function init() {
    ...
    function wireWabaConnectModals() {
        ...
        // silent=true is used for the EAGER boot on page load — we must not pop
        // an alert at a user who never opens the WABA modal. The click path calls
        // it non-silent so a genuine launch attempt still surfaces a load failure.
        const bootFb = (silent = false) => {
            if (window.FB) { window.FB.init({ appId, cookie: true, xfbml: true, version: graphVersion }); fbReady = true; return; }
            let tries = 0;
            const t = setInterval(() => {
                tries++;
                if (window.FB) {
                    ...
                    clearInterval(t);
                    if (!silent) alert('Meta SDK failed to load — refresh and try again.');
                }
            }, 200);
        };
        ...
        // Boot the Meta SDK NOW, not on the first tap. THIS is the mobile fix:
        // on a phone, FB.login() only opens its popup if it runs synchronously
        // inside the tap gesture. The old lazy path booted the SDK on click and
        // then called FB.login() from a setTimeout, so on mobile the popup was
        // blocked and the flow silently "went back" (desktop's lenient popup
        // rules hid it). Initialising here means fbReady is already true by tap
        // time, so runLaunch() takes its synchronous branch and the popup opens.
        bootFb(true);
    }
