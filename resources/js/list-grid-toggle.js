...
function wire(container) {
    ...
    // In the left-sidebar dashboard shell the content column is narrower, so
    // wide tables read better as cards — default to grid there. A saved user
    // choice always wins; topbar shell keeps the list default.
    const shellDefault = document.documentElement.dataset.userShell === 'sidebar' ? 'grid' : null;
    paint(container, saved || shellDefault || container.dataset.listGridView || 'list');
