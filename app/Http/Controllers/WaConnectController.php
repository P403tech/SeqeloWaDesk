...
class WaConnectController extends Controller
{
    ...
    /**
     * SMS path (Twilio SMS / MSG91). Connects an SMS number as a
     * WaProviderConfig provider='sms' row — the SAME device/provider store every
     * other channel uses, so SMS reuses the existing /devices connect UX.
     *
     * "We already have Twilio, use it": when the provider is Twilio and the SID
     * / token fields are blank, the workspace's existing provider='twilio'
     * (WhatsApp) credentials are inherited, so the operator only types the FROM
     * number. Secrets are encrypted in credentials_json; the non-secret
     * sms_provider / sender_id / DLT / rate live in meta_json; from_number is the
     * queryable phone_number column so inbound can find the row.
     *
     * NOT a WhatsApp engine — never touches is_primary / WorkspaceEngine::for(),
     * and 'sms' is not in allowed_send_methods, so it can't leak into WA pickers.
     */
    public function saveSms(Request $request): RedirectResponse
    {
        // Plan gate — SMS is a paid channel. This connect action lives in the
        // shared /connect/wa-store/* group (alongside Twilio/Baileys/WABA), so it
        // can't be wrapped by the plan: middleware; enforce inline instead. Throws
        // PlanLimitReachedException when the workspace's plan lacks access_sms.
        \App\Services\PlanLimitGuard::feature(Auth::user()?->currentWorkspace, 'access_sms');

        $data = $request->validate([
            'sms_provider'     => 'nullable|in:twilio,msg91',
            ...
            'rate_per_segment' => 'nullable|numeric|min:0',
        ...
        if (! $workspaceId) {
            return back()->withErrors(['from_number' => 'Pick or create a workspace first.'])->withInput();
        }

        $provider   = strtolower($data['sms_provider'] ?? 'twilio');
        $fromNumber = trim($data['from_number']);
        $accountSid = trim((string) ($data['account_sid'] ?? ''));
        $authToken  = trim((string) ($data['auth_token'] ?? ''));

        // Inherit the workspace's existing Twilio (WhatsApp) credentials when the
        // SMS form left them blank — the explicit "reuse our Twilio" behaviour.
        if ($provider === 'twilio' && ($accountSid === '' || $authToken === '')) {
            $tw = WaProviderConfig::query()->forWorkspace($workspaceId)->where('provider', 'twilio')->first();
            if ($tw) {
                $c = $tw->creds();
                $accountSid = $accountSid !== '' ? $accountSid : (string) ($c['account_sid'] ?? '');
                $authToken  = $authToken  !== '' ? $authToken  : (string) ($c['auth_token'] ?? '');
            }
        }

        // Verify the credentials before storing (never throws).
        $verify = \App\Services\Sms\SmsClient::verify($accountSid, $authToken, $provider);
        if (empty($verify['ok'])) {
            return back()->withErrors(['account_sid' => $verify['error'] ?? 'Could not verify the SMS credentials.'])->withInput();
        }

        // One row per (workspace, sms, number) — re-connecting the same number
        // updates it; a different number adds another SMS sender.
        $config = WaProviderConfig::firstOrNew([
            'workspace_id' => $workspaceId,
            'provider'     => 'sms',
            'phone_number' => $fromNumber,
        ...
        // Unified device cap — only on a brand-new sender.
        if (! $config->exists) {
            \App\Services\PlanLimitGuard::check(
                $user->currentWorkspace,
                'device_limit',
                \App\Models\Device::query()->forWorkspace($workspaceId, $user->id)->count()
                    + WaProviderConfig::query()->forWorkspace($workspaceId)
                        ->whereIn('provider', ['waba', 'twilio', 'sms'])->count(),
            );
        }
        ...
        $config->fill([
            ...
            'status'        => WaProviderConfig::STATUS_CONNECTED,
            'phone_number'  => $fromNumber,
            'display_label' => 'SMS · ' . ($provider === 'msg91' ? 'MSG91' : 'Twilio'),
            'connected_at'  => now(),
            // SMS is NOT a WhatsApp engine — never mark it primary, so it can't
            // affect WorkspaceEngine::for() / primaryForWorkspace() resolution.
            'is_primary'    => false,
            'meta_json'     => array_merge((array) $config->meta_json, [
                'sms_provider'     => $provider,
                'sender_id'        => trim((string) ($data['sender_id'] ?? '')),
                'dlt_template_id'  => trim((string) ($data['dlt_template_id'] ?? '')),
                'rate_per_segment' => (isset($data['rate_per_segment']) && $data['rate_per_segment'] !== null && $data['rate_per_segment'] !== '')
                    ? (string) $data['rate_per_segment'] : null,
        ...
        return redirect($dest)->with('status', 'SMS number connected. You can start sending now.');
    }
    ...
    public function nodeHeartbeat(Request $request): JsonResponse
    {
        ...
        // bookings with no attendance signal for review, auto-complete checked-in
        // ones. Cache-locked; no cron. (Reminders are the Node scheduler's job.)
        try {
            app(\App\Services\Appointments\AppointmentLifecycleSweeper::class)->sweep();
        } catch (\Throwable $e) {
            \Illuminate\Support\Facades\Log::error('[HEARTBEAT] appointment lifecycle sweep failed: ' . $e->getMessage());
        }

        // Auto-invoice delivery sweep — renders + sends pending invoices. Its own
        // 5-min cache gate, so the 30s heartbeat doesn't re-run it. No cron/queue.
        try {
            app(\App\Services\Invoice\InvoiceSweeper::class)->maybeRun();
        } catch (\Throwable $e) {
            \Illuminate\Support\Facades\Log::error('[HEARTBEAT] invoice sweep failed: ' . $e->getMessage());
        }
        ...
        // Tell Node whether campaigns are STILL draining so it can tick fast
        // (back-to-back parallel chunks) instead of waiting the full 30s — a big
        // multi-thousand blast then finishes in one continuous run rather than one
        // 20s chunk per 30s heartbeat. Active = we fired one this tick OR one is
        // mid-chunk ('running'). Best-effort; false on any error keeps the calm 30s.
        $campaignsActive = false;
        try {
            $campaignsActive = $sweptCampaigns > 0
                || \App\Models\WpCampaign::where('status', 'running')->exists();
        } catch (\Throwable $e) { $campaignsActive = $sweptCampaigns > 0; }

        return response()->json(['ok' => true, 'touched' => $touched, 'marked_stale' => $staleCount, 'campaigns_fired' => $sweptCampaigns, 'campaigns_active' => $campaignsActive, 'scheduled_retried' => $sweptScheduled, 'broadcasts_retried' => $sweptBroadcasts, 'deal_tasks_reminded' => $remindedTasks, 'stock_holds_released' => $releasedHolds]);
    }
