...
class AutoreplyController extends Controller
{
    ...
    {
        try {
            // Device scoping — a flow bound to a specific number (trigger_device_id)
            // shows only for THAT number; workspace-wide flows (null) always show.
            // Absent header = everything (back-compat).
            $pinned = (int) $request->attributes->get('app_device_id', 0);
            $flows = Flow::query()->forCurrentWorkspace()
                ->when($pinned > 0, fn ($q) => $q->where(function ($w) use ($pinned) {
                    $w->whereNull('trigger_device_id')->orWhere('trigger_device_id', $pinned);
                }))
                ->orderByDesc('id')
                ->get()
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        try {
            $reply = KeywordReply::create([
                ...
                // (boolean('status', true)) so a save without status stays enabled.
    ...
    public function update(Request $request, $id): JsonResponse
    {
        ...
        try {
            ...
            $pinned = (int) $request->attributes->get('app_device_id', 0);
            if (!$reply || ($pinned > 0 && (int) $reply->device_id !== $pinned)) {
                return response()->json([
                    'success' => false,
                    'message' => 'Autoreply not found',
                ], 404);
