...
class CampaignController extends Controller
{
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        try {
            ...
            // Delete-proof campaign cap — enforce the SAME increment-only usage
            // ledger the web store() uses so the mobile app can't bypass the
            // plan limit by delete+recreate either. Feature gate first, then the
            // numeric quota (live count seeds the meter once on first use).
            try {
                \App\Services\PlanLimitGuard::feature($user->currentWorkspace, 'campaign');
                \App\Services\PlanLimitGuard::checkQuota(
                    $user->currentWorkspace,
                    'total_campaigns_limit',
                    WpCampaign::PLAN_QUOTA_METRIC,
                    WpCampaign::where('workspace_id', $user->current_workspace_id)->count(),
                );
            ...
            // Device liveness (row was just refreshed by preflight's live Node
            // ping) — expose it so the app can show/verify the sender socket
            // WITHOUT trusting will_dispatch alone.
            $resolvedDevice = Device::query()->find($campaign->device_id);

            // Issue #1 — a 'now' campaign gets its send_date/send_time stamped to
            // the current moment and its schedule_type normalized, so the echoed
            // row reads as "scheduled for later" even though it dispatched
            // immediately. Surface the ORIGINAL intent explicitly so the client
            // never has to infer it: `sent_immediately` is the single boolean to
            // key on, and `requested_schedule_type` preserves exactly what was
            // sent ('now' | 'scheduled' | 'recurring').
            $data = $this->transformCampaign($campaign->fresh());
            $data['sent_immediately']        = !$isRecurring && $request->schedule_type === 'now' && $diag['ok'];
            $data['requested_schedule_type'] = (string) $request->schedule_type;
            ...
            return response()->json([
                ...
                'diagnostics'=> [
                    ...
                    // Live sender-socket state (from the preflight Node ping).
                    // If `device_connected` is false the send WON'T reach WhatsApp
                    // even when the row was created — reconnect the device first.
                    'device_connected'      => $resolvedDevice ? ($resolvedDevice->status === 'connected') : null,
                    'device_status'         => $resolvedDevice?->status,
                    'device_last_seen_at'   => optional($resolvedDevice?->last_seen_at)->toIso8601String(),
                ],
            ], 201);
        } catch (\Throwable $e) {
            DB::rollBack();
    ...
    private function preflightDispatch(WpCampaign $campaign): array
    {
        ...
        if (($campaign->provider ?? 'baileys') === 'baileys') {
            ...
            // LIVE liveness check — ask the Node bridge for the ACTUAL socket
            // state, not just the (possibly stale) DB row. The old check only
            // failed when status!='connected' AND !active, so a device whose row
            // still read active=true but whose Baileys socket had dropped sailed
            // through preflight → the campaign was "accepted" (will_dispatch=true)
            // and then silently never delivered (the exact campaign-189 symptom).
            //   true  → socket up, proceed
            //   false → Node says it's NOT connected → fail fast
            //   null  → bridge unreachable, can't prove it's down → fall open on
            //           the LIVE check but still honour a known-down DB row below.
            $live = $this->baileysLiveStatus($device);
            if ($live === false) {
                return ['ok' => false, 'error' => 'Sender device "' . ($device->device_name ?: ('#' . $device->id))
                    . '" is NOT connected to WhatsApp right now — its session is down. Reconnect it at /devices/'
                    . $device->id . '/qr, then resend.'];
            }
            if ($live === null && $device->status !== 'connected' && ! $device->active) {
                return ['ok' => false, 'error' => 'Sender device is not connected (last seen '
                    . ($device->last_seen_at ? $device->last_seen_at->diffForHumans() : 'never')
                    . '). Reconnect it at /devices/' . $device->id . '/qr first.'];
            }
        }
    ...
    /**
     * Live Baileys socket state for a device, straight from the Node bridge
     * (same /api/client-status/:phone endpoint the QR/pairing modal polls).
     *   true  → socket connected/ready
     *   false → Node answered and it is NOT connected
     *   null  → no bridge configured OR unreachable (state unknown)
     * Side-effect: refreshes the local devices row so the list + the next
     * preflight reflect reality (mirrors DevicesController::connectionStatus).
     */
    private function baileysLiveStatus(Device $device): ?bool
    {
        ...
        try {
            $res = \Illuminate\Support\Facades\Http::timeout(6)->acceptJson()
                ->withHeaders(['X-Node-Token' => node_token()])
                ->get($base . '/api/client-status/' . urlencode($phone));
            if (! $res->successful()) return null;

            $status    = (string) ($res->json('status') ?? '');
            $connected = ($res->json('isReady') === true)
                || in_array($status, ['connected', 'ready', 'open'], true);

            // Keep the row honest so the device list + next send are accurate.
            try {
                if ($connected && $device->status !== 'connected') {
                    $device->forceFill(['status' => 'connected', 'last_seen_at' => now()])->save();
                } elseif (! $connected && $device->status === 'connected') {
                    $device->forceFill(['status' => 'disconnected'])->save();
                }
            } catch (\Throwable $e) { /* row refresh is best-effort */ }

            return $connected;
        } catch (\Throwable $e) {
            return null; // bridge unreachable — unknown, don't block the send
        }
    }
