...
class Brand
{
    ...
    public static function billing(): array
    {
        ...
        return [
            ...
            // Logo printed on every platform invoice/receipt. Null → the view
            // falls back to the company wordmark.
            'logo'        => self::invoiceLogoUrl(),
        ];
    }

    /**
     * The logo to print on platform invoices / receipts. Prefers the dedicated
     * invoice logo the admin uploads at /admin/checkout-settings
     * (`billing.logo`); if none is set, falls back to the site's brand logo so
     * invoices are never logo-less on a branded install.
     */
