...
class CloudStorageManager
{
    ...
    private function buildDiskConfig(): ?array
    {
        ...
        // Defensive endpoint normalisation. Cloudflare R2 (and some S3 UIs)
        // show an "S3 API" URL that already ENDS in the bucket name, e.g.
        //   https://<acct>.r2.cloudflarestorage.com/dgtech-whatsapp-media
        // Admins paste that whole string into the Endpoint field. With
        // path-style addressing Laravel then appends the bucket a SECOND time,
        // producing public URLs like
        //   …/dgtech-whatsapp-media/dgtech-whatsapp-media/chat-media/x.jpg
        // which 404 — every media thumbnail shows broken. Strip a trailing
        // "/<bucket>" so the endpoint is the host only and the bucket is added
        // exactly once. No-op when the endpoint is already host-only.
        if ($endpoint) {
