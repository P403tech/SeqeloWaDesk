...
class OrderingService
{
    ...
    public function parseAndHold(int $wsId, string $customerPhone, string $text, ?string $groupCode = null, string $model = 'gpt-4o-mini'): array
    {
        ...
        return [
            ...
            // The RESOLVED customer text this parse actually used. For a VOICE or
            // PHOTO order the raw inbound had no text (the flow's {{user_message}}
            // was a bare voice-note id), so a later AI node "detect the language"
            // saw nothing and defaulted to English while the summary was already
            // localized — the exact mixed-language bug. The Node webhook executor
            // copies this back into {{user_message}} so downstream AI nodes see
            // the transcript and reply in the right language.
            'set_user_message' => (string) $effectiveText,
        ];
    }
    ...
    {
        $en = mb_strtolower(trim($this->gTranslate(trim($text), 'auto', 'en')['text']));
        $wordCount = count(preg_split('/\s+/', $en, -1, PREG_SPLIT_NO_EMPTY));

        // DECISIVE words are unambiguous even inside a longer sentence — a
        // customer who types "cancel", "confirm", "proceed", "place order" means
        // it. WEAK words (yes / no / ok / nope / stop) are only trusted in a
        // SHORT reply: in a long free-form message an INCIDENTAL "no"/"ok" must
        // NOT hijack the order. This was cancelling live orders — e.g. a random
        // Hindi message ("…tumhari mummy ko batana") machine-translated with an
        // incidental "no" matched the old bare `\bno\b` and killed the order.
        // Anything not decisively confirm/cancel → UNCLEAR → the caller re-asks
        // (never auto-cancels), which is the safe default for an order.
        $strongConfirm = (bool) preg_match('/\b(confirm|confirmed|confirming|proceed|place\s+(the\s+)?order|go\s+ahead)\b/u', $en);
        $strongCancel  = (bool) preg_match('/\b(cancel|canceled|cancelled|abort|discard)\b/u', $en);
        $weakConfirm   = (bool) preg_match('/\b(yes|yeah|yep|yup|ok|okay|sure|correct)\b/u', $en);
        $weakCancel    = (bool) preg_match('/\b(no|nope|stop|don\'?t)\b/u', $en);
        $short         = $wordCount <= 4;

        if ($strongConfirm) {
            $intent = 'confirm';                    // confirm wins ties
        } elseif ($strongCancel) {
            $intent = 'cancel';
        } elseif ($short && $weakConfirm) {
            $intent = 'confirm';
        } elseif ($short && $weakCancel) {
            $intent = 'cancel';
        } else {
            $intent = 'unclear';                    // long / ambiguous → re-ask, never auto-cancel
        }

        Log::info('[ORDERING] decideReply', ['en' => mb_substr($en, 0, 40), 'words' => $wordCount, 'intent' => $intent]);
