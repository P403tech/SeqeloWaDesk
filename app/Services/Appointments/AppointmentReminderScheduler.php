...
class AppointmentReminderScheduler
{
    public function schedule(Workspace $workspace, Appointment $appt, string $customerPhone, Carbon $remindAt, ?int $scheduleIdOverride = null, ?string $bodyOverride = null): bool
    {
        ...
        $body     = $bodyOverride ?? strtr($template, [
        ...
        // instead of stacking a second one. Negative range keeps it clear of
        // real scheduled_messages.id values. Booking-type multi-offset reminders
        // pass a widened id (§7.1: -2_000_000_000 - apptId*100 - offset_index) so
        // each offset gets its own slot and a reschedule replaces them all.
        $scheduleId = $scheduleIdOverride ?? (-1000000 - $appt->id);
