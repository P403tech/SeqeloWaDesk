...
import { generateWAMessageFromContent, generateMessageID, proto } from "@itsukichan/baileys";
...
export const pinMessage = async (req, res, app) => {
  ...
  try {
    ...
    // duration may arrive as "24h" / "7d" / "30d" / a raw seconds int. The pin
    // proto itself carries no duration field (WhatsApp defaults the pin window),
    // so we parse it only for logging/forward-compat — it doesn't change the send.
    const time = parseInt(duration, 10) || 86400;

    // The active @itsukichan/baileys fork's generateWAMessageContent has NO
    // `pin` convenience branch (unlike delete/keep/react), so the old
    // sock.sendMessage(jid,{pin:{...}}) fell through to media handling and threw
    // "Invalid media type" → 502. Build the pinInChatMessage proto directly and
    // relay it with edit='2' (the pin/unpin edit attribute the fork's own
    // messages-send.js sets for `content.pin`). Works for both pin and unpin.
    const pinType = wantPin
      ? proto.Message.PinInChatMessage.Type.PIN_FOR_ALL     // 1
      : proto.Message.PinInChatMessage.Type.UNPIN_FOR_ALL;  // 2
    console.log(`[NODE-PIN] → relaying pinInChatMessage jid=${jid} type=${pinType} time=${time} key=`, key);
    ...
    }, { userJid: sock.user?.id, messageId: generateMessageID() });
    await sock.relayMessage(jid, fullMsg.message, {
      messageId: fullMsg.key.id,
      additionalAttributes: { edit: '2' },
    });
    console.log(`[NODE-PIN] ✅ ${wantPin ? 'pinned' : 'unpinned'} target=${targetMessageId} jid=${jid} resultId=${fullMsg.key.id}`);
    return res.status(200).send({ ok: true, id: fullMsg.key.id || null });
  } catch (error) {
